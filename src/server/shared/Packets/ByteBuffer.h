...
class ByteBuffer
{
    public:
        ...
        ByteBuffer(size_t reserve) : _rpos(0), _wpos(0), _bitpos(8)
        {
        ...
        ByteBuffer(const ByteBuffer &buf) : _rpos(buf._rpos), _wpos(buf._wpos),
            _bitpos(buf._bitpos), _storage(buf._storage)
        {
        }
        ...
        */
        template <typename T> void PutBits(T value, int bits, size_t& pos, uint8& bitpos)
        {
            while (bits > 0)
            {
                if (bitpos == 8)
