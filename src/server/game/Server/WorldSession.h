...
class AfterInterRealmQueryHolder;
class Object;
class Player;
class Quest;
...
enum PetBattleCreateResult
{
    PET_BATTLE_CREATE_ERROR = 0,
    ...
    PET_BATTLE_CREATE_INCORRECT_PLAYER_TARGET = 8,
    PET_BATTLE_CREATE_TARGET_OUT_OF_RANGE = 9,
    PET_BATTLE_CREATE_INCORRECT_UNIT_TARGET = 10,
    // 11 - unk
    PET_BATTLE_CREATE_REJECT = 12,
    PET_BATTLE_CREATE_ALREADY_CREATED = 13,
    PET_BATTLE_CREATE_NO_PET_IN_SLOTS = 14,
    PET_BATTLE_CREATE_PETS_DIED = 15,
    PET_BATTLE_CREATE_NO_PET_IN_SLOTS_2 = 16,
    PET_BATTLE_CREATE_ATLAS_IS_BLOCKED = 17,
    PET_BATTLE_CREATE_PET_IN_COMBAT = 18,

    PET_BATTLE_CREATE_OK = 100, // custom value
...
enum DB2Types : uint32
{
    ...
    DB2_REPLY_ITEM_UPGRADE                  = 0x7006463B
};
...
class TransactionFuture
{
    public:
        ...
        void Start(SQLTransaction transaction)
        {
            m_Trans = transaction;
            m_Active = true;
        }
...
class WorldSession
{
    public:
        ...
        void SendAreaTriggerDenied(uint32 trigger_id);
        ...
        void SetSaveTransactionFuture(SQLTransaction trans) { _saveTransactionCallback.Start(trans); }
        ...
        void LoadReputationMods();
        void SaveReputationMods(SQLTransaction& trans);
        
        int32 GetReputationModifier(uint32 reputationId) const
        {
            auto itr = m_repMods.find(reputationId);
            if (itr != m_repMods.end())
                return itr->second;
            
            return 0;
        }

        void AddReputationMod(uint32 reputationId, int32 value)
        {
            auto itr = m_repMods.find(reputationId);
            if (itr == m_repMods.end())
            {
                m_repMods.insert({ reputationId, value });
                m_repModsChanged = true;
            }
        }
    ...
    public:                                                 // opcodes handlers
        ...
        void SendCustomizationResult(ResponseCodes code, ObjectGuid guid);
        ...
        void SendItemUpgradeReply(uint32 entry);
        ...
        void SendCharRenameResponse(uint8 p_ResultCode, ObjectGuid p_Guid = 0, std::string p_Name = "");
        ...
        void HandleStartWarGame(WorldPacket& p_Packet);
        ...
        void HandleSetLfgBonusFactionOpcode(WorldPacket& recvPacket);
        ...
        void HandleBattlePetStartPvPMatchMaking(WorldPacket& recv);
        void HandleBattlePetStopPvPMatchMaking(WorldPacket& recv);
        void HandleBattlePetAcceptPvpMatchMaking(WorldPacket& recv);
        void HandleBattlePetStartPvPDuel(WorldPacket& recv);
        void HandleBattlePetAcceptPvPDuel(WorldPacket& recv);

        void HandlePetBattleSetActivePet(WorldPacket& recv);
        void HandlePetBattleAction(WorldPacket& recv);
        void HandlePetBattleEnd(WorldPacket& recv);

        void SendPetBattleCreateError(PetBattleCreateResult result);
        PetBattleCreateResult CheckIsPetBattleAvailable();
        ...
        void LoadAfterInterRealmCallback(AfterInterRealmQueryHolder* holder);
    ...
    private:
        ...
        QueryResultHolderFuture _afterInterRealmCallback;
 
        TransactionFuture _saveTransactionCallback;
    ...
    private:
        ...
        time_t m_TimeLastMessageChatOpcode;
        uint32 m_uiAntispamMailSentCount;
        uint32 m_uiAntispamMailSentTimer;

        uint8 playerLoginCounter;
        
        typedef std::unordered_map<uint32, int> RepMods;
        RepMods m_repMods;
        
        bool m_repModsChanged;
};
#endif
/// @}
