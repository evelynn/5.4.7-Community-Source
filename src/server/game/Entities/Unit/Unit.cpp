...
ProcEventInfo::ProcEventInfo(Unit* actor, Unit* actionTarget, Unit* procTarget, uint32 typeMask, uint32 spellTypeMask, uint32 spellPhaseMask, uint32 hitMask, Spell const* spell, DamageInfo* damageInfo, HealInfo* healInfo)
...
Unit::Unit(bool isWorldObject): WorldObject(isWorldObject)
    ...
    , movespline(new Movement::MoveSpline(this))
    , i_AI(NULL)
    , i_disabledAI(NULL)
...
Unit::~Unit()
{
    ...
    {
        if (Player* player = m_sharedVision.front())
        {
...
void Unit::Update(uint32 p_time)
{
    ...
    if (m_logTimer >= 2000)
    {
        for (HealDamageLogList::iterator itr = m_healDamage.begin(); itr != m_healDamage.end();)
        {
            HealDamageLog& ptr = *itr;

            if (GetMSTimeDiffToNow(ptr.m_time) > 60 * IN_MILLISECONDS)
            {
                itr = m_healDamage.erase(itr);
            }
            else
                ++itr;
        }
    ...
    /*if (m_SendTransportMoveTimer != 0)
    {
        if (m_SendTransportMoveTimer <= p_time)
        {
            if (GetTransGUID())
            {
                ...
                init.MoveTo(m_movementInfo.transport.pos.m_positionX, m_movementInfo.transport.pos.m_positionY, m_movementInfo.transport.pos.m_positionZ);
                init.SetFacing(0.0f);
...
bool Unit::IsWithinMeleeRange(const Unit* obj, float dist) const
{
    ...
    if (GetTypeId() == TYPEID_PLAYER && (obj->GetTypeId() == TYPEID_PLAYER || (obj->IsPetGuardianStuff() && IS_PLAYER_GUID(obj->GetOwnerGUID()))))
    {
           ...
           if (player->HaveSpectators() && slot < MAX_AURAS)
            {
                ...
                uint64 casterID = 0;
                if (aura->GetCaster())
                    casterID = (aura->GetCaster()->IsPlayer()) ? aura->GetCaster()->GetGUID() : 0;
...
{
    victim->AddHealDamageLog(damage, DAMAGE_TAKE_LOG);
    AddHealDamageLog(damage, DAMAGE_DONE_LOG);
    ...
    if (damage > 1000000 && GetTypeId() == TYPEID_PLAYER && victim->GetTypeId() == TYPEID_UNIT && victim->ToCreature()->GetCreatureTemplate()->rank)
        sLog->OutTrinity("World Boss %u [%s] take more than 1M damage (%u) by player %u [%s] with spell %u", victim->GetEntry(), victim->GetName(), damage, GetGUIDLow(), GetName(), spellProto ? spellProto->Id : 0);
    ...
    {
        if (Aura* healingElixirs = plr->GetAura(122280))
        {
            if (plr->CountPctFromMaxHealth(35) > (plr->GetHealth() - damage))
            {
                if (plr->HasAura(134563))
                {
                    int32 bp = 15;
                    plr->CastCustomSpell(plr, 122281, &bp, NULL, NULL, true);
                    //plr->AddSpellCooldown(134563, 0, 18 * IN_MILLISECONDS);
                    plr->RemoveAura(134563);
    ...
    // Sparring - 116023
    if (plr && plr->getClass() == CLASS_MONK && plr->isInFront(this) && plr->GetSpecializationId() == SPEC_MONK_WINDWALKER && plr->getLevel() >= 42)
    {
        if (!plr->HasSpellCooldown(116023))
        {
            plr->AddAura(116033, plr);
            plr->AddSpellCooldown(116023, 0, 30 * IN_MILLISECONDS);
        }
    }
    ...
    {
        // Has set bonus aura and Bestial Wrath
        if (ToPlayer()->HasAura(144641) && ToPlayer()->HasAura(34471))
        {
            ToPlayer()->AddAura(144670, ToPlayer());

            // And apply on pet too
            if (Pet* hunterPet = ToPlayer()->GetPet())
            {
                // Bestial Wrath
                if (hunterPet->HasAura(19574))
                    ToPlayer()->AddAura(145737, hunterPet);
            }
        }
    }

    // Custom MoP Script - Item - Shaman T16 Enhancement 2P Bonus - 144962
    if (plr && IsPlayer() && ToPlayer()->getClass() == CLASS_SHAMAN && ToPlayer()->getLevel() == 90 && roll_chance_i(4))
    {
        // to prevent self trigger
        bool notSelfTriggered = !spellProto || spellProto && spellProto->Id != 73683 &&
                                               spellProto->Id !=  118470 && spellProto->Id !=  73682 &&
                                               spellProto->Id !=  118474 && spellProto->Id !=  73681 &&
                                               spellProto->Id !=  118472;

        if (notSelfTriggered && ToPlayer()->HasAura(144962) && ToPlayer()->HasAura(144964))
        {
            if (Player* shaman = this->ToPlayer())
            {
                Item* weapons[2];
                weapons[0] = shaman->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_MAINHAND);
                weapons[1] = shaman->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFFHAND);
                ...
                if (weapons[random])
                {
                    ...
                    uint32 furySpell = 0;
                    bool hasFuryTalent = shaman->HasAura(117012);
                    switch (weapons[random]->GetEnchantmentId(TEMP_ENCHANTMENT_SLOT))
                    {
                    ...
                    if (unleashSpell)
                        shaman->CastSpell(plr, unleashSpell, true);

                    if (furySpell)
                        shaman->CastSpell(plr, furySpell, true);
                }
            }
        }
    }

    if (plr && plr->HasSpell(5484) && plr != this && roll_chance_i(50))
        plr->ReduceSpellCooldown(5484, 1 * IN_MILLISECONDS);

    // Custom MoP Script - Soul Link
    if (this && this->IsPlayer() && damage > 0 && damagetype != NODAMAGE && this->getClass() == CLASS_WARLOCK && this->HasAura(108446))
    {
        const SpellInfo* soulLink = sSpellMgr->GetSpellInfo(108415);
        this->HealBySpell(this, soulLink, CalculatePct(damage, 3), false);

        // also we need to heal our pet
        if (Pet* pet = this->ToPlayer()->GetPet())
            this->HealBySpell(pet, soulLink, CalculatePct(damage, 3), false);
    }

    // Fix Unleashed Fury (117012) with Windfury Weapon
    if (this && this->getClass() == CLASS_SHAMAN && victim && damagetype == DIRECT_DAMAGE && this->HasAura(33757) && this->HasAura(118472) && this->HasAura(324) && this->HasAura(117012))
    {
        if (roll_chance_i(45))
            this->CastSpell(victim, 26364, true);
    }

    // Custom MoP Script - Shadow Blades
    if (this && this->getClass() == CLASS_ROGUE && victim && this->HasAuraType(SPELL_AURA_MOD_STEALTH) && this->HasAura(121471))
    {
        this->RemoveAurasByType(SPELL_AURA_MOD_STEALTH);
    }

    // Custom MoP Script - Subterfuge , should be called from normal hit
    if (this && this->getClass() == CLASS_ROGUE && victim && this->HasAura(108208) && this->HasAura(115191) && !this->HasAura(115192) && (damagetype == DIRECT_DAMAGE || this->HasAura(121471)))
        this->CastSpell(this, 115192, true);

    // Fix Nightstalker damage from stealth
    if (GetTypeId() == TYPEID_PLAYER  && this && this->getClass() == CLASS_ROGUE && victim && this->HasAura(130493))
    {
    ...
    if (plr && IsPlayer() && this->getClass() == CLASS_MONK && victim->HasAura(126060))
    {
    ...
    if (GetOwner() && GetTypeId() == TYPEID_UNIT && GetEntry() == 29264 && damage > 0)
    {
        ...
        CastCustomSpell(GetOwner(), 58879, &basepoints, NULL, NULL, true, 0, nullptr, GetGUID());
    }
    // Searing Flames - 77657 : Fire Elemental attacks or Searing Totem attacks
    Player* _owner = GetOwner() ? GetOwner()->ToPlayer() : nullptr;
    if (_owner && ((GetTypeId() == TYPEID_UNIT && GetEntry() == 15438 && !spellProto) || (isTotem() && GetEntry() == 2523)) && _owner->HasAura(77657) && _owner->GetSpecializationId() == SPEC_SHAMAN_ENHANCEMENT)
        _owner->CastSpell(_owner, 77661, true);

    // Stagger handler
    if (victim && victim->IsPlayer() && victim->getClass() == CLASS_MONK)
        if (!spellProto || (spellProto && spellProto->Id != STAGGER_DAMAGE_EFFECT && spellProto->DmgClass != SPELL_DAMAGE_CLASS_MAGIC))
            damage = victim->CalcStaggerDamage(victim->ToPlayer(), damage);

    // Temporal Shield - 115610
    if (victim->GetTypeId() == TYPEID_PLAYER && damage != 0 && victim->HasAura(115610))
    {
        int32 bp = damage;
    ...
    {
        if (cleanDamage->hitOutCome != MELEE_HIT_EVADE && 
            cleanDamage->hitOutCome != MELEE_HIT_MISS &&
            cleanDamage->hitOutCome != MELEE_HIT_DODGE &&
            cleanDamage->hitOutCome != MELEE_HIT_PARRY)
        {
    ...
    if (health <= damage)
    {
        if (victim->GetTypeId() == TYPEID_PLAYER && victim != this)
        {
            ...
            if (Player* killer = GetCharmerOrOwnerPlayerOrPlayerItself())
                killer->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_SPECIAL_PVP_KILL, 1, 0, 0, victim, false, true);
        }
...
uint32 Unit::CalcStaggerDamage(Player* victim, uint32 damage)
{
    ...
    if (victim->HasAura(120954) && victim->HasAura(117967))
        stagger -= 0.20f;
    ...
    int32 amount = CalculatePct(damage, ((1.0f - stagger) * 100.0f));
    if (stagger == 0.0f)
        amount = damage;

    const int32 ticks = 10;
    int32 bp = amount / ticks;

    /*STAGGER_DAMAGE_EFFECT and others will be automaticly removed when amount = 0*/
    if (auto aura = victim->GetAura(STAGGER_DAMAGE_EFFECT))
    {
        if (auto effect_1 = aura->GetEffect(EFFECT_1))
        {
            amount += effect_1->GetAmount();

            effect_1->SetAmount(amount);
        }
        if (auto effect_0 = aura->GetEffect(EFFECT_0))
        {
            bp = amount / ticks;
            effect_0->SetCanBeRecalculated(true);
            effect_0->ChangeAmount(bp);
        }
    }
    else
        victim->CastCustomSpell(victim, STAGGER_DAMAGE_EFFECT, &bp, &amount, NULL, true);

    const static uint32 staggerVisualEffects[3] = { LIGHT_STAGGER_EFFECT, MODERATE_STAGGER_EFFECT, HEAVY_STAGGER_EFFECT };

    uint32 prevId = 0;
    for (uint32 spell : staggerVisualEffects)
    {
        if (victim->HasAura(spell))
        {
            prevId = spell;
            break;
        }
    ...
    if (amount < (int32)victim->CountPctFromMaxHealth(3))
        spellId = LIGHT_STAGGER_EFFECT;
    else if ((int32)victim->CountPctFromMaxHealth(3) <= amount && amount <= (int32)victim->CountPctFromMaxHealth(6))
        spellId = MODERATE_STAGGER_EFFECT;
    else if (amount > (int32)victim->CountPctFromMaxHealth(6))
        spellId = HEAVY_STAGGER_EFFECT;

    if (prevId != spellId)
    {
        if (prevId > 0)
            victim->RemoveAura(prevId);

        victim->CastCustomSpell(victim, spellId, &bp, &amount, NULL, true);
    }
    else if (auto aura = victim->GetAura(spellId))
    {
        if (auto effect_0 = aura->GetEffect(EFFECT_0))
            effect_0->SetAmount(bp);
        if (auto effect_1 = aura->GetEffect(EFFECT_1))
            effect_1->SetAmount(amount);
...
bool Unit::CastSpell(SpellCastTargets const& p_Targets, SpellInfo const* p_SpellInfo, CustomSpellValues const* p_Values, TriggerCastFlags p_TriggerFlags /*= TRIGGERED_NONE*/, Item* p_CastItem /*= nullptr*/, AuraEffect const* p_TriggeredByAura /*= nullptr*/, uint64 p_OriginalCasterGUID /*= 0*/, float p_PeriodicDamageModifier /*= 0.0f*/)
{
    if (!p_SpellInfo)
        return false;

    // TODO: this is a workaround - not needed anymore, but required for some scripts :(
    if (!p_OriginalCasterGUID && p_TriggeredByAura)
        p_OriginalCasterGUID = p_TriggeredByAura->GetCasterGUID();

    Spell* l_Spell = new Spell(this, p_SpellInfo, p_TriggerFlags, p_OriginalCasterGUID);

    if (p_Values)
    {
        for (auto const& l_Pair : *p_Values)
            l_Spell->SetSpellValue(l_Pair.first, l_Pair.second);
    }

    l_Spell->m_CastItem = p_CastItem;
    l_Spell->SetPeriodicDamageModifier(p_PeriodicDamageModifier);
    return l_Spell->prepare(&p_Targets, p_TriggeredByAura);
}

bool Unit::CastSpell(Unit* p_Target, uint32 p_SpellId, bool p_Triggered, Item* p_CastItem /*= nullptr*/, AuraEffect const* p_TriggeredByAura /*= nullptr*/, uint64 p_OriginalCasterGUID /*= 0*/, float p_PeriodicDamageModifier /*= 0.0f*/)
{
    return CastSpell(p_Target, p_SpellId, p_Triggered ? TriggerCastFlags::TRIGGERED_FULL_MASK : TriggerCastFlags::TRIGGERED_NONE, p_CastItem, p_TriggeredByAura, p_OriginalCasterGUID, p_PeriodicDamageModifier);
}

bool Unit::CastSpell(Unit* p_Target, uint32 p_SpellId, TriggerCastFlags p_TriggerFlags /*= TRIGGERED_NONE*/, Item* p_CastItem /*= nullptr*/, AuraEffect const* p_TriggeredByAura /*= nullptr*/, uint64 p_OriginalCasterGUID /*= 0*/, float p_PeriodicDamageModifier /*= 0.0f*/)
{
    SpellInfo const* l_SpellInfo = sSpellMgr->GetSpellInfo(p_SpellId);
    if (!l_SpellInfo)
        return false;

    return CastSpell(p_Target, l_SpellInfo, p_TriggerFlags, p_CastItem, p_TriggeredByAura, p_OriginalCasterGUID, p_PeriodicDamageModifier);
}

bool Unit::CastSpell(Unit* p_Target, SpellInfo const* p_SpellInfo, bool p_Triggered, Item* p_CastItem /*= nullptr*/, AuraEffect const* p_TriggeredByAura /*= nullptr*/, uint64 p_OriginalCasterGUID /*= 0*/)
{
    return CastSpell(p_Target, p_SpellInfo, p_Triggered ? TriggerCastFlags::TRIGGERED_FULL_MASK : TriggerCastFlags::TRIGGERED_NONE, p_CastItem, p_TriggeredByAura, p_OriginalCasterGUID);
}

bool Unit::CastSpell(Unit* p_Target, SpellInfo const* p_SpellInfo, TriggerCastFlags p_TriggerFlags, Item* p_CastItem, AuraEffect const* p_TriggeredByAura, uint64 p_OriginalCasterGUID, float p_PeriodicDamageModifier /* = 0.0f */)
{
    SpellCastTargets l_Targets;
    l_Targets.SetUnitTarget(p_Target);

    return CastSpell(l_Targets, p_SpellInfo, nullptr, p_TriggerFlags, p_CastItem, p_TriggeredByAura, p_OriginalCasterGUID, p_PeriodicDamageModifier);
}

bool Unit::CastCustomSpell(Unit* p_Target, uint32 p_SpellId, int32 const* p_BP0, int32 const* p_BP1, int32 const* p_BP2, bool p_Triggered, Item* p_CastItem /*= nullptr*/, AuraEffect const* p_TriggeredByAura /*= nullptr*/, uint64 p_OriginalCasterGUID /*= 0*/)
{
    CustomSpellValues l_Values;
    if (p_BP0)
        l_Values.AddSpellMod(SpellValueMod::SPELLVALUE_BASE_POINT0, *p_BP0);

    if (p_BP1)
        l_Values.AddSpellMod(SpellValueMod::SPELLVALUE_BASE_POINT1, *p_BP1);

    if (p_BP2)
        l_Values.AddSpellMod(SpellValueMod::SPELLVALUE_BASE_POINT2, *p_BP2);

    return CastCustomSpell(p_SpellId, l_Values, p_Target, p_Triggered, p_CastItem, p_TriggeredByAura, p_OriginalCasterGUID);
}

bool Unit::CastCustomSpell(Unit* p_Target, uint32 p_SpellId, int32 const* p_BP0, int32 const* p_BP1, int32 const* p_BP2, int32 const* p_BP3, int32 const* p_BP4, int32 const* p_BP5, bool p_Triggered, Item* p_CastItem /*= nullptr*/, AuraEffect const* p_TriggeredByAura /*= nullptr*/, uint64 p_OriginalCasterGUID /*= 0*/)
{
    CustomSpellValues l_Values;
    if (p_BP0)
        l_Values.AddSpellMod(SpellValueMod::SPELLVALUE_BASE_POINT0, *p_BP0);
    ...
    if (p_BP3)
        l_Values.AddSpellMod(SpellValueMod::SPELLVALUE_BASE_POINT3, *p_BP3);

    if (p_BP4)
        l_Values.AddSpellMod(SpellValueMod::SPELLVALUE_BASE_POINT4, *p_BP4);

    if (p_BP5)
        l_Values.AddSpellMod(SpellValueMod::SPELLVALUE_BASE_POINT5, *p_BP5);

    return CastCustomSpell(p_SpellId, l_Values, p_Target, p_Triggered, p_CastItem, p_TriggeredByAura, p_OriginalCasterGUID);
}

bool Unit::CastCustomSpell(uint32 p_SpellId, SpellValueMod p_ValueMod, int32 p_Value, Unit* p_Target /*= nullptr*/, bool p_Triggered /*= true*/, Item* p_CastItem /*= nullptr*/, AuraEffect const* p_TriggeredByAura /*= nullptr*/, uint64 p_OriginalCasterGUID /*= 0*/)
{
    CustomSpellValues l_Values;
    l_Values.AddSpellMod(p_ValueMod, p_Value);

    return CastCustomSpell(p_SpellId, l_Values, p_Target, p_Triggered, p_CastItem, p_TriggeredByAura, p_OriginalCasterGUID);
}

bool Unit::CastCustomSpell(uint32 p_SpellId, CustomSpellValues const& p_Values, Unit* p_Target /*= nullptr*/, bool p_Triggered /*= true*/, Item* p_CastItem /*= nullptr*/, AuraEffect const* p_TriggeredByAura /*= nullptr*/, uint64 p_OriginalCasterGUID /*= 0*/)
{
    SpellInfo const* l_SpellInfo = sSpellMgr->GetSpellInfo(p_SpellId);
    if (!l_SpellInfo)
        return false;
    ...
    return CastSpell(l_Targets, l_SpellInfo, &p_Values, p_Triggered ? TriggerCastFlags::TRIGGERED_FULL_MASK : TriggerCastFlags::TRIGGERED_NONE, p_CastItem, p_TriggeredByAura, p_OriginalCasterGUID);
}

bool Unit::CastSpell(Position const& p_Pos, uint32 p_SpellId, bool p_Triggered, Item* p_CastItem /*= nullptr*/, AuraEffect const* p_TriggeredByAura /*= nullptr*/, uint64 p_OriginalCasterGUID /*= 0*/)
{
    return CastSpell(p_Pos.m_positionX, p_Pos.m_positionY, p_Pos.m_positionZ, p_SpellId, p_Triggered, p_CastItem, p_TriggeredByAura, p_OriginalCasterGUID);
}

bool Unit::CastSpell(float p_X, float p_Y, float p_Z, uint32 p_SpellId, bool p_Triggered, Item* p_CastItem /*= nullptr*/, AuraEffect const* p_TriggeredByAura /*= nullptr*/, uint64 p_OriginalCasterGUID /*= 0*/)
{
    SpellInfo const* l_SpellInfo = sSpellMgr->GetSpellInfo(p_SpellId);
    if (!l_SpellInfo)
        return false;
    ...
    SpellCastTargets l_Targets;
    l_Targets.SetDst(p_X, p_Y, p_Z, GetOrientation());

    return CastSpell(l_Targets, l_SpellInfo, nullptr, p_Triggered ? TriggerCastFlags::TRIGGERED_FULL_MASK : TriggerCastFlags::TRIGGERED_NONE, p_CastItem, p_TriggeredByAura, p_OriginalCasterGUID);
}

bool Unit::CastSpell(GameObject* p_Object, uint32 p_SpellId, bool p_Triggered, Item* p_CastItem /*= nullptr*/, AuraEffect* p_TriggeredByAura /*= nullptr*/, uint64 p_OriginalCasterGUID /*= 0*/)
{
    ...
    SpellCastTargets l_Targets;
    l_Targets.SetGOTarget(p_Object);

    return CastSpell(l_Targets, spellInfo, nullptr, p_Triggered ? TriggerCastFlags::TRIGGERED_FULL_MASK : TriggerCastFlags::TRIGGERED_NONE, p_CastItem, p_TriggeredByAura, p_OriginalCasterGUID);
}
...
void Unit::CalculateMeleeDamage(Unit* victim, uint32 damage, CalcDamageInfo* damageInfo, WeaponAttackType attackType)
{
    ...
    if (int32(damageInfo->damage) > 0)
    {
        ...
        damageInfo->damageSchoolMask = CalcAbsorbResist(damageInfo->target, SpellSchoolMask(damageInfo->damageSchoolMask), DIRECT_DAMAGE, damageInfo->damage, &damageInfo->absorb, &damageInfo->resist);
    ...
    {
            if (victim && _player->getLevel() >= 85 && _player->getClass() == CLASS_SHAMAN && _player->GetSpecializationId(_player->GetActiveSpec()) == SPEC_SHAMAN_ENHANCEMENT && _player->HasAura(131554))
            {
                bool frostflameProc = false;
                ...
                for (int i = 0; i < 2; i++)
                {
                    if (!weapons[i])
                        continue;
                    // If weapon doesn't have Flametongue enchant
                    if (weapons[i]->GetEnchantmentId(TEMP_ENCHANTMENT_SLOT) != 5)
                        continue;
                    
                    // If enchant on main hand - damage should be from it, and the same for off hand
                    if ((i == 0 && attackType == BASE_ATTACK) || (i == 1 && attackType == OFF_ATTACK))
                        frostflameProc = true;
                }

                if (frostflameProc)
                    _player->CastSpell(victim, 147732, true);
            }
    }
}
...
void Unit::DealMeleeDamage(CalcDamageInfo* damageInfo, bool durabilityLoss)
{
    ...
    damageInfo->damage = DealDamage(victim, damageInfo->damage, &cleanDamage, DIRECT_DAMAGE, SpellSchoolMask(damageInfo->damageSchoolMask), NULL, durabilityLoss);
...
{
    EmotesEntry const* l_EmoteEntry = sEmotesStore.LookupEntry(p_EmoteID);
    if (!l_EmoteEntry)
    {
        SetUInt32Value(EUnitFields::UNIT_NPC_EMOTESTATE, 0);
        return;
    }
    ...
    switch (l_EmoteEntry->EmoteType)
    {
        case 0:
        {
            WorldPacket l_Data(Opcodes::SMSG_EMOTE, 4 + 8);
        ...
        case 1:
        case 2:
            SetUInt32Value(EUnitFields::UNIT_NPC_EMOTESTATE, p_EmoteID);
            break;
        default:
            break;
    }
...
SpellSchoolMask Unit::CalcAbsorbResist(Unit* victim, SpellSchoolMask schoolMask, DamageEffectType damagetype, uint32 const damage, uint32 *absorb, uint32 *resist, SpellInfo const* spellInfo)
{
    ...
    for (AuraEffectList::iterator itr = vSchoolAbsorbCopy.begin(); (itr != vSchoolAbsorbCopy.end()) && (dmgInfo.GetDamage() > 0); ++itr)
    {
        ...
        if (absorbAurEff->GetAmount() >= 0)
        {
            ...
            if (absorbAurEff->GetAmount() <= 0 && aura->GetId() != 115069 && aura->GetId() != 114039) // Custom MoP Script - Stance of the Sturdy Ox shoudn't be removed at any damage
    ...
    if (victim != this)
    {
        ...
        for (AuraEffectList::iterator itr = vSplitDamagePctCopy.begin(), next; (itr != vSplitDamagePctCopy.end()) &&  (dmgInfo.GetDamage() > 0); ++itr)
        {
            ...
            // Hand of Sacrifice don't damage paladin with Glyph of Hand of Sacrifice
            if (caster && caster->getClass() == CLASS_PALADIN && (*itr)->GetSpellInfo()->Id == 6940 && caster->HasAura(146957))
                continue;
            ...
            log.damage = splitted - split_absorb;
            log.absorb = split_absorb;
            log.HitInfo = SPELL_HIT_TYPE_UNK1 | SPELL_HIT_TYPE_UNK3 | SPELL_HIT_TYPE_SPLIT_DAMAGE;
    ...
    // Custom MoP Script - Flameglow (140468)
    if (dmgInfo.GetDamage() >= 0 && victim->getClass() == CLASS_MAGE && victim->HasSpell(140468))
    {
...
void Unit::CalcHealAbsorb(Unit* victim, const SpellInfo* healSpell, uint32 &healAmount, uint32 &absorb)
{
    ...
    for (AuraEffectList::const_iterator i = vHealAbsorb.begin(); i != vHealAbsorb.end() && RemainingHeal > 0; ++i)
    {
        ...
        absorbAurEff->GetBase()->CallScriptEffectAbsorbHealHandlers((*i), aurApp, RemainingHeal, tempAbsorb, defaultPrevented);
        currentAbsorb = tempAbsorb;
        ...
        if (absorbAurEff->GetSpellInfo()->Id == 110214 || 
            absorbAurEff->GetSpellInfo()->Id == 110598)
        {
            if (victim)
            {
...
void Unit::AttackerStateUpdate (Unit* victim, WeaponAttackType attType, bool extra)
{
    ...
    else
    {
        ...
        // Hack fix for send damage information to log from treants of Force of Nature
        if (TempSummon* temp = damageInfo.attacker->ToTempSummon())
        {
            if (temp->GetEntry() == ENTRY_TREANT_FERAL || temp->GetEntry() == ENTRY_TREANT_GUARDIAN)
            {
                if (Unit* owner = temp->GetSummoner())
                {
                    const SpellInfo* autospell = sSpellMgr->GetSpellInfo(6603);
                    owner->SendSpellNonMeleeDamageLog(victim, autospell->Id, damageInfo.damage, autospell->GetSchoolMask(), 0, 0, false, 0, false);
                }
            }
        }

        // Hack fix for Snakes from Hunter Trap, if they deal very big damage
        if (TempSummon* snake = this->ToTempSummon())
        {
            if (snake && (snake->GetEntry() == 19833 || snake->GetEntry() == 19921) && damageInfo.damage > 1000)
...
{
    WorldPacket data(SMSG_ATTACK_STOP, 17);
...
{
    int32 value = GetTotalAuraModifier(SPELL_AURA_MOD_BLOCK_CRIT_CHANCE);

    // Fix Mastery : Critical Block - Increase critical block chance
    bool hasMastery = HasAura(76857);
    if (hasMastery)
        value += int32(GetFloatValue(PLAYER_MASTERY) * 2.2f);
...
SpellMissInfo Unit::MeleeSpellHitResult(Unit* victim, SpellInfo const* spell)
{
    ...
    if (attType == RANGED_ATTACK)
    {
        ...
        // Blind is raged attack
        // But cannot be dodged
        if (spell->Id == 2094)
            canDodge = false;
    }
...
SpellMissInfo Unit::MagicSpellHitResult(Unit* victim, SpellInfo const* spell)
{
    ...
    // Cloak of Shadow doesn't give immunity to Flare
    if (spell->Id == 94528 && victim->isAlive() && victim->getClass() == CLASS_ROGUE && victim->HasAura(31224))
        return SPELL_MISS_NONE;
    ...
    if (spell->IsInterruptSpell() || spell->HasCustomAttribute(SPELL_ATTR0_CU_AURA_CC))
    {
   ...
   // Roll chance
    ...
    // Hack additional check for Deterrence
    if (victim->getClass() == CLASS_HUNTER && victim->getLevel() >= 78 && victim->HasAuraType(SPELL_AURA_IGNORE_HIT_DIRECTION) && victim->HasAura(19263) && !victim->HasAura(83495))
        return SPELL_MISS_DEFLECT;
...
SpellMissInfo Unit::SpellHitResult(Unit* victim, SpellInfo const* spell, bool CanReflect)
{
    ...
    if (CanReflect)
    {
        // From 5.3 patch Pets and Guardians spells are not reflected.
        ...
        // Fix Glyph of Mirrored Blades with Deterrence (148467) - changed by Crouching Tiger, Hidden Chimaera
        if (victim->getClass() == CLASS_HUNTER && victim->getLevel() >= 78 && victim->HasAura(148467) && victim->HasAura(83495))
            reflectchance += 100;

        // Hack fix for Static Charge
        if (spell->Id == 118905)
            reflectchance = 0;
    ...
    switch (spell->DmgClass)
    {
        ...
        case SPELL_DAMAGE_CLASS_NONE:
        {
            ...
            // Warrior Charge
            if (!(spell->SpellFamilyName == SPELLFAMILY_WARRIOR && (spell->SpellFamilyFlags[0] & 0x1)))
            {
                int32 deflect_chance = victim->GetTotalAuraModifier(SPELL_AURA_DEFLECT_SPELLS);
                if (roll_chance_i(deflect_chance))
                    return SPELL_MISS_DEFLECT;
...
void Unit::InterruptSpell(CurrentSpellTypes spellType, bool withDelayed, bool withInstant)
{
        ...
        && (withInstant || spell->GetCastTime() > 0))
    {
        ...
        // handle cast for spectator addon
        if (Player* tmpPlayer = ToPlayer())
        {
            if (curSpellInfo->CastTimeEntry && !curSpellInfo->IsPassive())
            {
...
Aura* Unit::_TryStackingOrRefreshingExistingAura(SpellInfo const* newAura, uint32 effMask, Unit* caster, int32* baseAmount /*= NULL*/, Item* castItem /*= NULL*/, uint64 casterGUID /*= 0*/, int32 castItemLevel /*= -1*/)
{
    ...
    if (!newAura->IsMultiSlotAura())
    {
        ...
        if (castItem)
        {
            ...
            castItemLevel = castItem->GetOwner() ? castItem->GetOwner()->GetEquipItemLevelFor(castItem->GetTemplate(), castItem) : -1;
        }
        ...
        if (foundAura != nullptr)
        {
            ...
            if (foundAura->GetId() != 980 && !foundAura->IsRemoved())
                foundAura->ModStackAmount(1);

            // Agony is refreshed at manual reapply
            if (foundAura->GetId() == 980 && !foundAura->IsRemoved())
            {
                foundAura->ModStackAmount(0); // Damage snapshot
...
void Unit::_AddAura(UnitAura* aura, Unit* caster)
{
    ...
    // Lifebloom (33763) in Incarnation: Tree of Life (33891) isn't a single target spell
    if (caster && aura->GetSpellInfo()->Id == 33763 && caster->HasAura(33891))
    ...
    if (aura->IsSingleTarget())
    {
        ...
        for (Unit::AuraList::iterator itr = scAuras.begin(); itr != scAuras.end();)
        {
            ...
            {
                // Lifebloom - if has more then 2 seconds left we should move our effects
                if ((*itr)->GetId() == 33763)
                {
                    if ((*itr)->GetDuration() >= 2000)
                    {
                        int32 stackAmount = (*itr)->GetStackAmount();
    ...
    if (caster)
        caster->RegisterAuraApplyTime(aura);
}
...
void Unit::_RemoveNoStackAurasDueToAura(Aura* aura)
{
    ...
    for (AuraApplicationMap::iterator i = m_appliedAuras.begin(); i != m_appliedAuras.end(); ++i)
    {
        ...
        Aura* _base = i->second->GetBase();
        if (aura->CanStackWith(_base))
            continue;

        // Hack fix remove seal by consecration
        if ((_base->GetId() == 105361 ||
            _base->GetId() == 101423 ||
            _base->GetId() == 31801 ||
            _base->GetId() == 20165 ||
            _base->GetId() == 20164)
        ...
        if (spellProto->Id == 123267 &&
            (_base->GetId() == 81206 ||
            _base->GetId() == 81208 ||
            _base->GetId() == 81209 || spellProto->Id == 120583))
            continue;

        // Hack fix for Glyph of Shadow Word: Death
        if ((spellProto->Id == 110485 || spellProto->Id == 123266 ||
            spellProto->Id == 123267 || spellProto->Id == 81206 ||
            spellProto->Id == 81208 || spellProto->Id == 81209 || spellProto->Id == 15473) && 
            _base->GetId() == 120583)
            continue;
...
bool Unit::IsHighestExclusiveAura(Aura const* aura, bool removeOtherAuraApplications /*= false*/)
{
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        auto aurEff = aura->GetEffect(i);
        if (!aurEff)
            continue;

        AuraType const auraType = AuraType(aurEff->GetSpellInfo()->Effects[i].ApplyAuraName);
        AuraEffectList const& auras = GetAuraEffectsByType(auraType);
        for (Unit::AuraEffectList::const_iterator itr = auras.begin(); itr != auras.end();)
        {
            AuraEffect* existingAurEff = (*itr);
            ++itr;
...
void Unit::RemoveOwnedAura(AuraMap::iterator &i, AuraRemoveMode removeMode)
{
    ...
    if (auto caster = aura->GetCaster())
        caster->UnRegisterAuraApplyTime(aura);
...
void Unit::RemoveAurasDueToSpellBySteal(uint32 spellId, uint64 casterGUID, Unit* stealer)
{
    for (AuraMap::iterator iter = m_ownedAuras.lower_bound(spellId); iter != m_ownedAuras.upper_bound(spellId);)
    {
        ...
        if (aura->GetCasterGUID() == casterGUID)
        {
            ...
            std::string data[MAX_SPELL_EFFECTS];
            uint32 effMask = 0;
            ...
            for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
            {
                if (aura->GetEffect(i))
                {
                    ...
                    data[i] = aura->GetEffect(i)->GetData();
                    effMask |= (1<<i);
            ...
            else
            {
                ...
                if (newAura != nullptr)
                {
                    ...
                    newAura->SetLoadedState(aura->GetMaxDuration(), int32(dur), stealCharge ? 1 : aura->GetCharges(), 1, recalculateMask, &damage[0], &data[0]);
...
void Unit::RemoveAurasByType(AuraType auraType, uint64 casterGUID, Aura* exceptAura, uint32 exceptAuraId, bool negative, bool positive)
{
    for (AuraEffectList::iterator iter = m_modAuras[auraType].begin(); iter != m_modAuras[auraType].end();)
    {
        ...
        // Hack Fix : Glyph of Shadow Word: Death (120583)
        if (aura->GetSpellInfo()->Id == 32409)
            continue;
...
void Unit::RemoveDKPresences()
{
    ...
    // Blood Precense
    if (HasAura(48263))
        this->RemoveAura(48263);
    // Frost Precense
    if (HasAura(48266))
        this->RemoveAura(48266);
    // Unholy Precense
    if (HasAura(48265))
        this->RemoveAura(48265);
}
...
void Unit::RemoveNonPassivesAuras()
{
    ...
    {
        auto base = aurAppIter->second->GetBase();
        if (!base->IsPassive() && !base->GetSpellInfo()->HasAura(SPELL_AURA_SPELL_MAGNET))
        {
            switch (base->GetId())
            {
                case 110310:
...
void Unit::RemoveAllAurasOnDeath()
{
    ...
    {
        Aura const* aura = iter->second->GetBase();
        if (!aura->IsPassive() && !aura->IsDeathPersistent() && aura->GetId() != 32727)
    ...
    for (AuraApplicationMap::const_iterator iter = m_appliedAuras.begin(); iter != m_appliedAuras.end(); ++iter)
    {
        ...
        for (uint8 i = 0; i < MAX_EFFECTS; ++i)
        {
            if (aura->GetSpellInfo()->GetEffectMechanicMask(i) & mechanic_mask)
            {
                if (iter->second && aura)
                    if (aura->GetEffect(i))
                        list.push_back(aura->GetEffect(i));
            }
        }
    }
...
uint32 Unit::GetAuraCount(uint32 spellId) const
{
    ...
    for (AuraApplicationMap::const_iterator itr = m_appliedAuras.lower_bound(spellId); itr != m_appliedAuras.upper_bound(spellId); ++itr)
    {
        ...
        if (!aura->GetStackAmount())
            ...
            count += (uint32)aura->GetStackAmount();
    }
    return count;
}
...
bool Unit::HasAura(uint32 spellId, uint64 casterGUID, uint64 itemCasterGUID, uint32 reqEffMask) const
{
    if (this == nullptr)
    {
        sLog->OutTrinity("Unit::HasAura(spellId: %u) this == nullptr", spellId);
        return false;
    }
...
bool Unit::HasNegativeAuraWithInterruptFlag(uint32 flag, uint64 guid)
{
    ...
    {
        Aura const* aura = (*iter)->GetBase();
        if (!(*iter)->IsPositive() && aura->GetSpellInfo()->AuraInterruptFlags & flag && (!guid || aura->GetCasterGUID() == guid))
            return true;
    }
    return false;
}
...
bool Unit::HasAuraWithMechanic(uint32 mechanicMask)
{
    for (AuraApplicationMap::iterator iter = m_appliedAuras.begin(); iter != m_appliedAuras.end(); ++iter)
    {
        ...
        Aura* auracheck = iter->second->GetBase();
        if (!auracheck)
            continue;

        SpellInfo const* spellInfo  = auracheck->GetSpellInfo();
...
int32 Unit::GetTotalAuraModifierByEquippedItemClass(AuraType auratype, int32 itemClass) const
{
    ...
    for (AuraEffectList::const_iterator i = mTotalAuraList.begin(); i != mTotalAuraList.end(); ++i)
    {
        if ((*i)->GetSpellInfo()->EquippedItemClass == itemClass)
        {
...
int32 Unit::GetTotalAuraModifier(AuraType auratype) const
{
    ...
    AuraEffectList const& mTotalAuraList = GetAuraEffectsByType(auratype);
    for (AuraEffectList::const_iterator i = mTotalAuraList.begin(); i != mTotalAuraList.end(); ++i)
         if (!sSpellMgr->AddSameEffectStackRuleSpellGroups((*i)->GetSpellInfo(), (*i)->GetAmount(), SameEffectSpellGroup, (*i)->GetAuraType()))
             modifier += (*i)->GetAmount();
...
int32 Unit::GetTotalAuraModifier(AuraType p_AuraType, std::function<bool(AuraEffect* /*p_AuraEffect*/, int32& /*p_Amount*/)> const& p_AuraFunc) const
{
    std::map<SpellGroup, int32> SameEffectSpellGroup;
    int32 modifier = 0;

    for (AuraEffect* l_Effect : GetAuraEffectsByType(p_AuraType))
    {
        int32 l_Amount = l_Effect->GetAmount();
        if (!p_AuraFunc(l_Effect, l_Amount))
            continue;

        if (!sSpellMgr->AddSameEffectStackRuleSpellGroups(l_Effect->GetSpellInfo(), l_Amount, SameEffectSpellGroup, l_Effect->GetAuraType()))
            modifier += l_Amount;
    }
...
int32 Unit::GetTotalAuraModifierByMiscBMask(AuraType auratype, uint32 misc_mask) const
{
    std::map<SpellGroup, int32> SameEffectSpellGroup;
    int32 modifier = 0;
...
float Unit::GetTotalAuraMultiplierByMiscBMask(AuraType auratype, uint32 misc_mask) const
{
    std::map<SpellGroup, int32> SameEffectSpellGroup;
    float multiplier = 1.0f;
...
int32 Unit::GetMaxPositiveAuraModifierByMiscBMask(AuraType auratype, uint32 misc_mask, AuraEffect const* except) const
{
    int32 modifier = 0;
    ...
    for (AuraEffectList::const_iterator i = mTotalAuraList.begin(); i != mTotalAuraList.end(); ++i)
    {
        if (except != (*i) && (*i)->GetMiscValueB()& misc_mask && (*i)->GetAmount() > modifier)
            modifier = (*i)->GetAmount();
    }
...
int32 Unit::GetMaxNegativeAuraModifierByMiscBMask(AuraType auratype, uint32 misc_mask) const
{
    int32 modifier = 0;
    ...
    for (AuraEffectList::const_iterator i = mTotalAuraList.begin(); i != mTotalAuraList.end(); ++i)
    {
        if ((*i)->GetMiscValueB()& misc_mask && (*i)->GetAmount() < modifier)
            modifier = (*i)->GetAmount();
    }
...
int32 Unit::GetTotalAuraModifierByMiscValue(AuraType auratype, int32 misc_value, int32 misc_valueB) const
{
    std::map<SpellGroup, int32> SameEffectSpellGroup;
    int32 modifier = 0;
    ...
    for (AuraEffectList::const_iterator i = mTotalAuraList.begin(); i != mTotalAuraList.end(); ++i)
    {
        if ((*i)->GetMiscValue() == misc_value && (!misc_valueB || (*i)->GetMiscValueB() == misc_valueB))
            if (!sSpellMgr->AddSameEffectStackRuleSpellGroups((*i)->GetSpellInfo(), (*i)->GetAmount(), SameEffectSpellGroup))
                modifier += (*i)->GetAmount();
    }
...
void Unit::RemoveAllAreasTrigger()
{
    while (!m_AreaTrigger.empty())
        m_AreaTrigger.front()->Remove(false);
}
...
void Unit::AddGameObject(GameObject* gameObj)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER && gameObj->GetSpellId())
    {
        ...
        if (createBySpell && createBySpell->Attributes & SPELL_ATTR0_DISABLED_WHILE_ACTIVE)
        {
            ...
            ToPlayer()->AddChargeCooldown(createBySpell);
        }
    }
}
...
void Unit::ProcDamageAndSpell(Unit* victim, uint32 procAttacker, uint32 procVictim, uint32 procExtra, uint32 amount, uint32 absorb, WeaponAttackType attType, SpellInfo const* procSpell, SpellInfo const* procAura, Aura const* procEffect, Spell const* spell)
{
     // Not much to do if no flags are set.
    if (procAttacker)
        ProcDamageAndSpellFor(false, victim, procAttacker, procExtra, attType, procSpell, amount, absorb, procAura, procEffect, spell);
    // Now go on with a victim's events'n'auras
    // Not much to do if no flags are set or there is no victim
    if (victim && victim->isAlive() && procVictim)
        victim->ProcDamageAndSpellFor(true, this, procVictim, procExtra, attType, procSpell, amount, absorb, procAura, procEffect, spell);
}
...
void Unit::SendSpellMiss(Unit* target, uint32 spellID, SpellMissInfo missInfo)
{
    ...
    ObjectGuid vguid = target->GetGUID();
    ...
    // ClientSpellLogMissEntry bits
    {
        uint8 bitOrder[8] = {3, 6, 5, 4, 7, 1, 0, 2};
        data.WriteBitInOrder(vguid, bitOrder);
        data.WriteBit(false); // ClientSpellLogMissDebug
    }
...
void Unit::SendAttackStateUpdate(CalcDamageInfo* damageInfo)
{
    ...
    data << int32(overkill < 0 ? -1 : overkill);            // Overkill
    ...
    data.put<int32>(pos - sizeof(int32), uint32(data.wpos() - pos));
...
bool Unit::HandleHasteAuraProc(Unit* victim, uint32 damage, AuraEffect* triggeredByAura, SpellInfo const* /*procSpell*/, uint32 /*procFlag*/, uint32 /*procEx*/, uint32 cooldown)
{
    ...
    if (cooldown && GetTypeId() == TYPEID_PLAYER)
        ToPlayer()->AddSpellCooldown(triggered_spell_id, 0, cooldown * IN_MILLISECONDS);
...
bool Unit::HandleAuraProcOnPowerAmount(Unit* victim, uint32 /*damage*/, AuraEffect* triggeredByAura, SpellInfo const *procSpell, uint32 procFlag, uint32 /*procEx*/)
{
...
//victim may be NULL
bool Unit::HandleDummyAuraProc(Unit* victim, uint32 damage, uint32 absorb, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlag, uint32 procEx, Aura const* procEffect)
{
    ...
    CustomSpellValues customValues;
    ...
    switch (dummySpell->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            switch (dummySpell->Id)
            {
                case 139190:
                {
                    ...
                    auto _aura = GetAura(139189);
                    if (_aura && _aura->GetStackAmount() >= 5)
                    {
                        _aura->Remove();
                        triggered_spell_id = 139195;
                    }
                    else
                        triggered_spell_id = 139189;
                ...
                case 138957:
                {
                    if (HasAura(138960) || ToPlayer()->HasSpellCooldown(138960))
                        return false;

                    auto _aura = GetAura(138958);
                    if (_aura && _aura->GetStackAmount() >= 9)
                    {
                        _aura->Remove();
                        CastSpell(this, 138960, true, castItem, triggeredByAura);
                        ToPlayer()->AddSpellCooldown(138960, 0, 25 * IN_MILLISECONDS);
                        return true;
                    }
                    else
                ...
                {
                    if (!victim || !(ToPlayer() || ToPet()) || !(victim->ToPlayer() || victim->ToPet()) || victim == this || !damage || (procEx & PROC_EX_INTERNAL_HOT))
                        return false;
                ...
                case 120694:
                    if (Unit* l_Owner = GetOwner())
                        EnergizeBySpell(l_Owner, dummySpell->Id, dummySpell->Effects[effIndex].BasePoints, POWER_FOCUS);
                    break;
                ...
                case 35429:
                {
                    target = SelectNearbyTarget(victim, NOMINAL_MELEE_RANGE, true);
                    if (!target)
                        return false;
                ...
                case 28305:
                {
                    ...
                    {
                        target->EnergizeBySpell(target, 123051, int32(float(target->CountPctFromMaxMana(7)) / 4.0f), POWER_MANA);
                        return false;
                    }
        ...
        case SPELLFAMILY_MAGE:
        {
            switch (dummySpell->Id)
            {
                ...
                case 12846: // Mastery: Ignite
                {
                    ...
                    if (procSpell->Id != 133 && procSpell->Id != 44614 && 
                        procSpell->Id != 108853 && procSpell->Id != 2948 &&
                        procSpell->Id != 11366 && procSpell->Id != 2120)
                        return false;

                    triggered_spell_id = 12654;

                    float value = GetFloatValue(PLAYER_MASTERY) * 1.5f / 100.0f;
                    basepoints0 = (damage * value) / 2;
                ...
                case 44448: // Improved Hot Streak
                {
                    ...
                    if (victim->GetGUID() == this->GetGUID())
                        return false;
                ...
                {
                    RemoveAurasByType(SPELL_AURA_HASTE_SPELLS, 0, nullptr, true, false);
                ...
                case 64411:
                {
                    ...
                    if (AuraEffect* aurEff = victim->GetAuraEffect(64413, EFFECT_0))
        ...
        case SPELLFAMILY_WARRIOR:
        {
            switch (dummySpell->Id)
            {
                ...
                {
                    if (GetTypeId() != TYPEID_PLAYER || !damage || !(procEx & (PROC_EX_NORMAL_HIT|PROC_EX_CRITICAL_HIT)))
                        return false;

                    if (procSpell && (procSpell->Id == 76858 || procSpell->Id == 26654))
                        return false;
                    
                    float customChance = ToPlayer()->GetFloatValue(PLAYER_MASTERY) * 2.2f;
                    if (!roll_chance_f(customChance))
                        return false;
                ...
                case 12292:
                {
                    ...
                    bp += victim->GetRemainingPeriodicAmount(GetGUID(), 113344, SPELL_AURA_PERIODIC_DAMAGE);
        ...
        case SPELLFAMILY_WARLOCK:
        {
            switch (dummySpell->Id)
            {
                case 108370:
                {
                    if (GetTypeId() != TYPEID_PLAYER || !damage || !procSpell)
                        return false;

                    static const int32 s_Percents[3] = { 20, 10, 10 };

                    int32 index = ToPlayer()->GetSpecializationId() - SPEC_WARLOCK_AFFLICTION;
                    if (index < 0 || index >= 3)
                        return false;

                    int32 amount = CalculatePct(damage, s_Percents[index]);

#define APPLY_AURA(_target) \
    if (auto _effect = _target->GetAuraEffect(108366, EFFECT_0)) \
    { \
        int32 newAmount = std::min(_effect->GetAmount() + amount, int32(_target->CountPctFromMaxHealth(15))); \
        _effect->ChangeAmount(newAmount); \
        if (_effect->GetBase()) \
            _effect->GetBase()->RefreshDuration(); \
    }\
    else \
    {    \
        int32 newAmount = std::min(amount, int32(_target->CountPctFromMaxHealth(15))); \
        CastCustomSpell(_target, 108366, &newAmount, nullptr, nullptr, true); \
    }
...
#undef APPLY_AURA
                ...
                case 108563: // Backlash
                    // after specific quest with 'green fire' reward
                    if (HasAura(101508))
                        triggered_spell_id = 140076;
                    else
                ...
                case 56218: // Glyph of siphone life
                {
                    if (!isAlive() || !damage)
                        return false;

                    // proc from direct spell damage if spellId is 131740
                    if ((procFlag & PROC_FLAG_DONE_SPELL_MAGIC_DMG_CLASS_NEG))
                    {
                        if (!procSpell || procSpell->Id != 131740)
                            return false;
                    }
        ...
        case SPELLFAMILY_PRIEST:
        {
            ...
            if (dummySpell->Id == 47515)
            {
                ...
                if (Aura* divineAegis = target->GetAura(47753))
                    basepoints0 += divineAegis->GetEffect(0)->GetAmount();

                int32 maxValue = CalculatePct(this->GetMaxHealth(), 60);
                if (basepoints0 > maxValue)
                    basepoints0 = maxValue;
            ...
            switch (dummySpell->Id)
            {
                ...
                case 15286:
                {
                    ...
                    std::list<Unit*> membersInRange;

                    GetPartyMembers(members, true);

                    uint32 amountTargets = 0;
                    for (auto member : members)
                    {
                        if (member && IsInRange(member, 0.0f, 39.0f, false))
                        {
                    ...
                    bp = int32(bp / amountTargets);

                    SpellInfo const* l_ProcSpell = sSpellMgr->GetSpellInfo(15290);

                    if (l_ProcSpell == nullptr)
                        return false;

                    HealBySpell(this, l_ProcSpell, bp, false);
                    for (auto l_Target : membersInRange)
                    {
                        if (l_Target != nullptr)
                            HealBySpell(l_Target, l_ProcSpell, bp, false);
                    }
                ...
                case 92297:
                {
                    ...
                    if (ToPlayer()->HasSpellCooldown(92297))
                        return false;
                    ...
                    ToPlayer()->AddSpellCooldown(92297, 0, 500);    ///< Prevent multiple procs
                    break;
                }
        ...
        case SPELLFAMILY_DRUID:
        {
            switch (dummySpell->Id)
            {
                ...
                case 124974:
                {
                    if (!procSpell || !damage || !victim || procSpell->IsAffectingArea())
                        return false;

                    basepoints0 = CalculatePct(damage, 25);

                    bool isHealingDone = procFlag & (PROC_FLAG_DONE_SPELL_MAGIC_DMG_CLASS_POS) || procEx & PROC_EX_INTERNAL_HOT;
                    if (isHealingDone)
                    {
                        CastCustomSpell(this, 124991, &basepoints0, nullptr, nullptr, true, castItem, triggeredByAura, originalCaster); // damage
                        CastCustomSpell(this, 124988, &basepoints0, nullptr, nullptr, true, castItem, triggeredByAura, originalCaster); // heal
                    }

                    bool isDamageDone = procFlag & (PROC_FLAG_DONE_SPELL_MELEE_DMG_CLASS | PROC_FLAG_DONE_SPELL_MAGIC_DMG_CLASS_NEG) || procEx & PROC_EX_INTERNAL_DOT;
                    if (isDamageDone)
                        CastCustomSpell(this, 124988, &basepoints0, nullptr, nullptr, true, castItem, triggeredByAura, originalCaster); // heal

                    return true;
                }
                case 135700:// Omen of Clarity
                {
                    if (!procSpell)
                        return false;
                ...
                {
                    if (GetTypeId() != TYPEID_PLAYER || getLevel() < 90)
                        return false;
                ...
                case 28719: // Healing Touch (Dreamwalker Raiment set)
                {
                    ...
                    {
                        basepoints0 = int32(CalculatePct(itr->Cost, 30));
                        break;
                    }
                    target = this;
                ...
                case 17007: ///< Leader of the Pack
                {
                    ...
                    if (l_Player->HasSpellCooldown(17007))
                        return false;

                    CastSpell(this, 34299, true); // Heal
                    CastSpell(this, 68285, true); // Energize

                    l_Player->AddSpellCooldown(17007, 0, 6 * IN_MILLISECONDS); // 6s ICD
                    return true;
                }
            }
            // Living Seed
            if (dummySpell->SpellIconID == 2860)
            {
                ...
                if (AuraEffect* livingSeedAura = victim->GetAuraEffect(triggered_spell_id, EFFECT_0, GetGUID()))
                    basepoints0 += livingSeedAura->GetAmount();

                int32 maxHealth = CountPctFromMaxHealth(50);
        ...
        case SPELLFAMILY_ROGUE:
        {
            switch (dummySpell->Id)
            {
                case 51701: // Honor Among Thieves
                {
                    ...
                    {
                        player->AddSpellCooldown(51701, 0, 2 * IN_MILLISECONDS);
                        player->CastSpell(spellTarget, 51699, true);

                        // Anticipation proc
                        if (player->GetComboPoints() == 5 && player->HasAura(51701) && player->HasAura(114015))
                            player->AddAura(115189, player);
                    }
                ...
                case 114015:// Anticipation
                {
                    ...
                    for (uint8 i = 0; i < procSpell->EffectCount; ++i)
                    {
                        ...
                        {
                            newCombo += procSpell->Effects[i].CalcValue(this, &procSpell->Effects[i].BasePoints, nullptr);
                            break;
                        }
                    }
        ...
        case SPELLFAMILY_HUNTER:
        {
            switch (dummySpell->SpellIconID)
            {
                ...
                case 3409:
                {
                    if (procSpell->Id != 56641 && procSpell->Id != 76663) // not steady shot
                    {
                    ...
                    if (triggeredByAura->GetAmount() == 0 && procSpell->Id != 76663)
                    {
                        triggeredByAura->SetAmount(2);
                        return false;
                    }
                    else if (triggeredByAura->GetAmount() == 3)
                    {
                        triggeredByAura->SetAmount(0);
                        return false;
                    }
            ...
            switch (dummySpell->Id)
            {
                case 53397:
                {
                    if (!damage || !ToPet() || !GetOwner() || !roll_chance_i(15))
                        return false;

                    Player* _owner = GetOwner()->ToPlayer();
                    if (!_owner || !ToPet()->IsPermanentPetFor(_owner) || !_owner->HasAura(53253))
                        return false;

                    _owner->CastSpell(_owner, 53398, true);
                    break;
                }
        ...
        case SPELLFAMILY_PALADIN:
        {
            switch (dummySpell->Id)
            {
                ...
                case 144580:
                {
                    if (!damage || !triggeredByAura)
                        return false;
                    if (!HasAura(498)) // Divine Protection 
                        return false;

                    auto nextAmount = triggeredByAura->GetAmount() + int32(damage * 0.075f);
                    triggeredByAura->SetAmount(nextAmount);
                    break;
                }
                ...
                case 92272:
                {
                    ...
                    for (auto itr : procSpell->SpellPowers)
                    {
                        if (itr->CostBasePercentage)
                        {
                            ...
                            int32 mana = int32(0.2f * CalculatePct(GetCreateMana(), itr->CostBasePercentage));
                ...
                case 76669:
                {
                    ...
                    switch (procSpell->Id)
                    {
                        ...
                        case 86452:
                        {
                            int32 Mastery = int32(this->ToPlayer()->GetFloatValue(PLAYER_MASTERY) * 1.25f);

                            basepoints0 = CalculatePct(damage, Mastery);
        ...
        case SPELLFAMILY_SHAMAN:
        {
            switch (dummySpell->Id)
            {
                ...
                case 55447: ///< Glyph of Flame Shock
                {
                    ...
                    if (!procSpell || procSpell->Id != 8050)
                        return false;

                    HealBySpell(this, dummySpell, CalculatePct(damage, triggerAmount));
                    break;
                }
            }
            // Frozen Power
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
            switch (dummySpell->Id)
            {
                ...
                case 144907: // Item - Death Knight T16 DPS 4P Bonus
                {
                    if (GetTypeId() != TYPEID_PLAYER)
                        return false;
                    if (!procSpell || procSpell->Id != 47632)
                        return false;

                    if (auto pet = ToPlayer()->GetPet())
                    {
                        if (auto pet_aura = pet->GetAura(63560))
                        {
                            auto currentValue = pet_aura->GetDuration();
                            auto currentValueMax = pet_aura->GetMaxDuration();

                            pet_aura->SetMaxDuration(currentValueMax + 2000);
                            pet_aura->SetDuration(currentValue + 2000);
                        }
                    }

                    target = this;
                    triggered_spell_id = 144909;
                    break;
                }
                case 144909:
                {
        ...
        case SPELLFAMILY_MONK:
        {
            switch (dummySpell->Id)
            {
                    ...
                    for (Unit* controlled : m_Controlled)
                    {
                        switch (controlled->GetEntry())
                        {
                            ...
                            case 69791: // Fire
                            {
                                ...
                                {
                                    if (getVictim() != spirit->getVictim())
                                    {
                                        if (!spirit->HasSpellCooldown(procSpell->Id) && !spirit->HasUnitState(UNIT_STATE_CASTING) && !spirit->GetCurrentSpell(CURRENT_CHANNELED_SPELL))
                                        {
                                            spirit->CastSpell(spirit->getVictim(), procSpell->Id, TRIGGERED_IGNORE_POWER_AND_REAGENT_COST);
                                            spirit->_AddCreatureSpellCooldown(procSpell->Id, time(nullptr) + 1);
                                        }
                                    }
                                }
                                break;
                ...
                case 127722: // Serpent Zeal
                case 130324: // Eminence
                {
                    if (!victim)
                        return false;
                    ...
                    if (dummySpell->Id == 130324)
                    {
                        if (!procSpell)
                            return false;

                        if (!(procFlag & PROC_FLAG_DONE_SPELL_MELEE_DMG_CLASS) && procSpell->Id != 117952)
                            return false;

                        if (procSpell->Id == 120278 || procSpell->Id == 120274 || procSpell->Id == 107270)
                            return false;
                    }
                    else if (dummySpell->Id == 127722)
                    {
                        if (!(procFlag & PROC_FLAG_DONE_MELEE_AUTO_ATTACK))
                            return false;
                    }

                    basepoints0 = 0.25f * damage;

                    target = this;
                    triggered_spell_id = 126890;

                    Creature* l_SerpentStatue = nullptr;
                    for (Unit* l_Controlled : m_Controlled)
                    {
                        if (l_Controlled->GetEntry() == 60849)
                        {
                    ...
                    if (l_SerpentStatue)
                    {
                        int32 bp = basepoints0;
                        l_SerpentStatue->CastCustomSpell(l_SerpentStatue, 117895, &bp, NULL, NULL, true, NULL, NULL, GetGUID());
                    }
                    break;
                }
            }
    ...
    switch (dummySpell->Id)
    {
        case 138238:
        {
            ...
            if (auto aura = AddAura(138242, this))
            {
                int32 dur = (GetPower(POWER_HOLY_POWER) >= 3 ? 3 : GetPower(POWER_HOLY_POWER) + 1) * 5000;
    ...
    if (basepoints0)
        customValues.AddSpellMod(SpellValueMod::SPELLVALUE_BASE_POINT0, basepoints0);

    if (basepoints1 >= 0)
        customValues.AddSpellMod(SpellValueMod::SPELLVALUE_BASE_POINT1, basepoints1);

    CastCustomSpell(triggered_spell_id, customValues, target, true, castItem, triggeredByAura, originalCaster);
    return true;
}
...
bool Unit::HandleProcTriggerSpell(Unit* victim, uint32 damage, uint32 absorb, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlags, uint32 procEx, bool& applyCooldown)
{
    // Get triggered aura spell info
    SpellInfo const* auraSpellInfo = triggeredByAura->GetSpellInfo();
            ...
            case SPELLFAMILY_GENERIC:
                switch (auraSpellInfo->Id)
                {
                    case 105574:
                    {
                        ...
                        auto highestStat = ToPlayer()->GetHighestStat();
                        switch (highestStat)
                        {
                ...
                switch (auraSpellInfo->Id)
                {
                    ...
                    case 53238:
                    {
                        ...
                        switch (procSpell->Id)
                        {
                            ...
                            case 53209: ///< Chimaera Shot
                            case 56641: ///< Steady Shot
                                trigger_spell_id = 63468;
                                basepoints0 = CalculatePct(int32(damage), triggerAmount) / 8;
            ...
            case SPELLFAMILY_SHAMAN:
            {
                switch (auraSpellInfo->Id)
                {
                    ...
                    case 23572:
                    {
                        ...
                        for (auto itr : procSpell->SpellPowers)
                        {
                            basepoints0 = int32(CalculatePct(itr->Cost, 35));
                            break;
                        }
    ...
    switch (auraSpellInfo->Id)
    {
        ...
        case 105574:
            applyCooldown = false;
            break;
        case 146315: // Item - Heals Proc Int
        {
            if (!IsPlayer() || ToPlayer()->GetRoleForGroup() != ROLES_HEALER)
                return false;
            break;
        }
        ...
        {
            if (HasAura(53257))
                CastCustomSpell(53257, SPELLVALUE_AURA_STACK, 1, this, true);
            else
                CastCustomSpell(53257, SPELLVALUE_AURA_STACK, 2, this, true);
        ...
        case 126154:
        {
            if (victim && damage >= victim->GetMaxHealth() * 0.35)
                if (victim->HasAura(126154))
                    victim->RemoveAura(126154);
        ...
        case 54943: // Glyph of Blessed Life
        {
            ...
            ToPlayer()->AddSpellCooldown(54943, 0, 20 * IN_MILLISECONDS);
        ...
        case 109306:// Trill of the Hunt
        {
            ...
            for (auto itr : procSpell->SpellPowers)
            {
                ...
                if (itr->powerType != POWER_FOCUS && procSpell->DmgClass != SPELL_DAMAGE_CLASS_RANGED)
                    continue;
            ...
            if (!focus && procSpell->Id != 34026)
                return false;

            // Cobra Shot
            if (procSpell->Id == 77767)
                return false;

            // Kill Shot
            if (procSpell->Id == 53351)
                return false;
            ...
            // Mastery proc
            if (procSpell->Id == 76663)
                return false;

            // Aimed Shot (instant with no power cost)
            if (procSpell->Id == 82928)
                return false;

            // Explosive Shot with no power cost
            if (procSpell->Id == 53301 && GetSpentPowerCost() == 0)
                return false;
        ...
        case 131564:// Arcane Intensity
        {
            ...
            // Can proc just from Arcane Shot
            if (procSpell->Id != 3044)
                return false;

            break;
        }
        case 118976:// Viper Venom
        {
            if (GetTypeId() != TYPEID_PLAYER)
                return false;
        ...
        case 5118:  // Aspect of the Cheetah
        {
            ...
            // some health pct effects proc as PROC_FLAG_TAKEN_SPELL_RANGED_DMG_CLASS rather than PROC_FLAG_TAKEN_SPELL_NONE_DMG_CLASS_POS, becoase of SPELL_DAMAGE_CLASS_RANGED
            // so lets just skip this spells
            if (!ToPlayer() || (procSpell && procSpell->HasEffect(SPELL_EFFECT_HEAL_PCT)))
                return false;

            // Glyph of Aspect of the Cheetah
            if (HasAura(119462))
            {
                CastSpell(this, 5118, true);
                RemoveAura(5118);
                return false;
            }
        ...
        case 108215:// Paralytic Poison
        {
            ...
            if (auraSpellInfo->Id == 108215)
                if (victim && victim->HasAura(113953))
                    return false;
        ...
        case 49509: // Scent of Blood
        {
            ...
            if (ToPlayer()->GetSpecializationId() != SPEC_DK_BLOOD)
                return false;
        ...
        case 110803:// Lightning Shield (Symbiosis)
        {
            ...
            ToPlayer()->AddSpellCooldown(110804, 0, 4 * IN_MILLISECONDS);
        ...
        case 144593://Item - Paladin T16 Retribution 4P Bonus
        {
            ...
            for (auto itr : procSpell->SpellPowers)
            {
                if (itr->powerType != POWER_HOLY_POWER)
                    continue;
        ...
        case 131633:// Item - Paladin PvP Set Retribution 2P Bonus
        {
            if (!procSpell)
                return false;
        ...
        case 134563:// Healing Elixirs (heal trigger)
        {
            if (!procSpell)
                return false;
            ...
            // Can proc just from Brews
            if (procSpell->Id != 115203 && procSpell->Id != 115399 && procSpell->Id != 137562 && 
                procSpell->Id != 115308 && procSpell->Id != 119582 && procSpell->Id != 115294 && 
                procSpell->Id != 116680 && procSpell->Id != 115288 && procSpell->Id != 116740)
                return false;

            break;
        }
        case 56343: // Lock and Load
        {
            if (HasAura(67544)) // Lock and Load Marker
                return false;

            if (procSpell && procSpell->Id == 3674 && !roll_chance_i(20)) // If coming from Black Arrow, there's only 20% chance
                return false;

            AddAura(56453, this); // Apply Lock and Load
            AddAura(67544, this); // Apply Lock and Load marker
            break;
        }
        ...
        case 138301:// Priest T15 Healer 4P Bonus
        {
            if (!procSpell)
                return false;

            // Penance - 47750, Circle of Healing - 34861
            if (procSpell->Id != 47750 && procSpell->Id != 34861)
                return false;
        ...
        {
            if (procSpell && procSpell->Id == 20271)
                return false;
        ...
        {
            if (procSpell && (procSpell->Id == 755 || procSpell->Id == 1949))
                return false;
        ...
        case 33371: // Glyph of Mind Spike
        {
            ...
            if (GetCurrentSpellCastTime(73510) == 0)
                return false;
        ...
        case 12950: // Meat Cleaver
        {
            ...
            if (procSpell->Id != 1680)
                return false;

            if (ToPlayer()->HasSpellCooldown(12950))
                return false;

            // custom cooldown, to prevent multiprocs
            ToPlayer()->AddSpellCooldown(12950, 0, 1 * IN_MILLISECONDS);
        ...
        case 35551: // Combat Potency
        {
            ...
            if (procSpell && procSpell->Id != 86392 && procSpell->Id != 121474)
                return false;
            ...
            {
                float offHandSpeed = static_cast<float>(GetAttackTime(OFF_ATTACK)) / IN_MILLISECONDS;
        ...
        case 138216:
        {
            if (!(procEx & PROC_EX_DODGE)) ///< Only dodged attacks
                return false;

            if (!HasAura(132402)) ///< Savage Defense
                return false;

            break;
        }
        case 146953: // Glyph of Fortuitous Sphere
        {
            if (GetHealthPct() < 25.0f || (100.f * (GetHealth() - damage) / GetMaxHealth()) > 25.0f)
                return false;
    ...
    switch (trigger_spell_id)
    {
        ...
        case 35095:
        {
            ...
            for (auto itr : procSpell->SpellPowers)
            {
                if (itr->powerType != POWER_MANA || (itr->Cost == 0 && itr->CostBasePercentage == 0))
                    continue;
...
void Unit::setPowerType(Powers new_powertype)
{
    ...
    SetUInt32Value(UNIT_FIELD_OVERRIDE_DISPLAY_POWER, 0);
    ...
    if (apply)
    {
        ...
        if (GetTypeId() == TYPEID_PLAYER)
        {
            ...
            if (spellInfo && (spellInfo->Attributes & SPELL_ATTR0_DISABLED_WHILE_ACTIVE))
            {
                ...
                ToPlayer()->AddChargeCooldown(spellInfo);
            }
        }
    }
    else
...
int32 Unit::DealHeal(Unit* victim, uint32 addhealth, SpellInfo const* spellProto)
{
    ...
    victim->AddHealDamageLog(addhealth, HEAL_TAKE_LOG);
    AddHealDamageLog(addhealth, HEAL_DONE_LOG);
    ...
    else if (unit && unit->GetTypeId() == TYPEID_PLAYER && unit->getLevel() >= 80 && addhealth > 0 && spellProto && spellProto->ShouldTriggerGiftOfTheSerpent() && unit->HasAura(117907))
    {
...
Unit* Unit::GetMagicHitRedirectTarget(Unit* victim, SpellInfo const* spellInfo)
{
    ...
    bool groundingTotemRedirect = false;
    ...
    for (Unit::AuraEffectList::const_iterator itr = magnetAuras.begin(); itr != magnetAuras.end(); ++itr)
    {
        ...
        {
            // Hack fix for Grounding Totem, should redirect Freezing Trap too
            if (magnet->isTotem() && spellInfo->Id == 3355)
                groundingTotemRedirect = true;

            if ((spellInfo->CheckExplicitTarget(this, magnet) == SPELL_CAST_OK
                || groundingTotemRedirect)
...
void Unit::SendEnergizeSpellLog(Unit* victim, uint32 spellID, uint32 damage, Powers powerType)
{
    ...
    // if (hasPowerData)
    //{
    //    for (int i = 0; i < powerDataCount; ++i)
    //    {
    //        data << uint32 << uint32
    //    }
...
void Unit::EnergizeBySpell(Unit* victim, uint32 spellID, int32 damage, Powers powerType)
{
    ...
    // reset regen timers
    if (Player* player = ToPlayer())
        player->ResetRegenTimer(powerType);
}
...
uint32 Unit::SpellDamageBonusDone(Unit* victim, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack)
{
    ...
    if (spellProto->HasAttribute(SPELL_ATTR3_NO_DONE_BONUS) || spellProto->HasAttribute(SPELL_ATTR6_NO_DONE_PCT_DAMAGE_MODS))
        return pdamage;
    if (spellProto->Id == 74196 || spellProto->Id == 50273)
        return pdamage;
    if (spellProto->Id == 118594)
        return pdamage;
    ...
    if (spellProto->Id == 122233 || spellProto->Id == 96172 || spellProto->Id == 70890) // Crimson Tempest, Hand of Light and Scourge Strike
        return pdamage;
    ...
    {
        float power = 0.0f;
        if (auto _player = ToPlayer())
            power = _player->GetFloatValue(PLAYER_FIELD_PVP_POWER_DAMAGE);
        else if (isPet() && GetOwner())
        {
            if (auto _player = GetOwner()->ToPlayer())
                power = _player->GetFloatValue(PLAYER_FIELD_PVP_POWER_DAMAGE);
        }
        AddPct(DoneTotalMod, power);
    }
    
    // Custom MoP Script
    // 76658 - Mastery : Essence of the Viper
    if (GetTypeId() == TYPEID_PLAYER && spellProto
    && (spellProto->SchoolMask == SPELL_SCHOOL_MASK_FIRE || spellProto->SchoolMask == SPELL_SCHOOL_MASK_FROST || spellProto->SchoolMask == SPELL_SCHOOL_MASK_NATURE || spellProto->SchoolMask == SPELL_SCHOOL_MASK_SHADOW || spellProto->SchoolMask == SPELL_SCHOOL_MASK_ARCANE) && HasAura(76658))
    {
    ...
    if (GetTypeId() == TYPEID_PLAYER && spellProto
        ...
        || spellProto->Id == 108686) && HasAura(77220))
    {
    ...
    if (GetTypeId() == TYPEID_PLAYER && spellProto && (spellProto->Id == 17877 || spellProto->Id == 116858) && HasAura(77220))
    {
    ...
    // Harvest Life - 108371
    // Increases the damage of Drain Life - 689, Drain Life (Demonic) - 103990
    if (GetTypeId() == TYPEID_PLAYER && spellProto && (spellProto->Id == 689 || spellProto->Id == 103990) && HasAura(108371))
        AddPct(DoneTotalMod, 50.0f);

    // Custom MoP Script
    // 76808 - Mastery : Executioner
    if (GetTypeId() == TYPEID_PLAYER && spellProto && (spellProto->Id == 2098 || spellProto->Id == 1943 || spellProto->Id == 121411) && HasAura(76808))
    {
        float Mastery = GetFloatValue(PLAYER_MASTERY) * 3.0f;
        AddPct(DoneTotalMod, Mastery);
    }
    ...
    if (GetTypeId() == TYPEID_PLAYER && spellProto && (spellProto->Id == 2818 || spellProto->Id == 8680 || spellProto->Id == 113780 || spellProto->Id == 32645 || spellProto->Id == 79136) && pdamage != 0 && HasAura(76803))
    {
    ...
    else if (isPet() || GetEntry() == ENTRY_WILD_IMP)
    {
        Unit* owner = GetOwner();
    ...
    if (spellProto && GetTypeId() == TYPEID_PLAYER && getClass() == CLASS_MAGE)
    {
        if (HasAura(76547))
        {
    ...
    if (GetTypeId() == TYPEID_PLAYER && victim && pdamage != 0 && spellProto && spellProto->SchoolMask == SPELL_SCHOOL_MASK_SHADOW && getClass() == CLASS_DEATH_KNIGHT)
    {
        if (HasAura(77515))
        {
    ...
    if (GetTypeId() == TYPEID_PLAYER && spellProto && (spellProto->Id == 117014 || spellProto->SchoolMask == SPELL_SCHOOL_MASK_FIRE || spellProto->SchoolMask == SPELL_SCHOOL_MASK_FROST || spellProto->SchoolMask == SPELL_SCHOOL_MASK_NATURE))
    {
        if (HasAura(77223))
        {
            float Mastery = GetFloatValue(PLAYER_MASTERY) * 2.0f;
    ...
    if (GetTypeId() == TYPEID_PLAYER && spellProto && (spellProto->SchoolMask == SPELL_SCHOOL_MASK_NATURE || spellProto->Id == 78674) && HasAura(77492) && HasAura(48517)) // Solar Eclipse
    {
        float Mastery = GetFloatValue(PLAYER_MASTERY) * 1.87f;
        AddPct(DoneTotalMod, Mastery);
    }
    else if (GetTypeId() == TYPEID_PLAYER && spellProto && (spellProto->SchoolMask == SPELL_SCHOOL_MASK_ARCANE || spellProto->Id == 78674) && HasAura(77492) && HasAura(48518)) // Lunar Eclipse
    {
        float Mastery = GetFloatValue(PLAYER_MASTERY) * 1.87f;
        AddPct(DoneTotalMod, Mastery);
    }
    ...
    if (GetTypeId() == TYPEID_PLAYER && pdamage != 0 && ToPlayer()->GetSpecializationId() == SPEC_MAGE_FROST && spellProto && getLevel() >= 24)
    {
    ...
    /// Wild Imps Firebolt got bonus from Grimoire of Sacrifice
    if (spellProto->Id == 104318)
    {
        if (Unit* l_Owner = GetOwner())
        {
            if (l_Owner->HasAura(108503)) ///< Grimoire of Sacrifice
                AddPct(DoneTotalMod, 20);
        }
    }
    ...
    if (!(spellProto->AttributesEx6 & SPELL_ATTR6_NO_DONE_PCT_DAMAGE_MODS) && !spellProto->IsRankOf(sSpellMgr->GetSpellInfo(12162)))
    {
        ...
        for (AuraEffect const* l_Effect : mModDamagePercentDone)
        {
            SpellInfo const* l_SpellInfo = l_Effect->GetSpellInfo();
            if (l_Effect->GetMiscValue() & spellProto->GetSchoolMask())
            {
                // Incarnation : Chosen of Elune
                // Increases Arcane and Nature damage if in Eclipse
                if (l_SpellInfo->Id == 122114 && !(HasAura(48517) || HasAura(48518) || HasAura(112071)))
                    continue;

                /// Deep Wounds is not affected by some auras
                if (spellProto->Id == 115767)
                {
                    switch (l_SpellInfo->Id)
                    {
                        case 12712: ///< Seasoned Soldiers
                        case 81099: ///< Single-Minded Fury
                            continue;

                        default:
                            break; ///< Breaks only the case
                    }
                }

                if (l_SpellInfo->EquippedItemClass == -1)
                    AddPct(DoneTotalMod, l_Effect->GetAmount());
                else if (!(l_SpellInfo->AttributesEx5 & SPELL_ATTR5_SPECIAL_ITEM_CLASS_CHECK) && (l_SpellInfo->EquippedItemSubClassMask == 0))
                    AddPct(DoneTotalMod, l_Effect->GetAmount());
                else if (IsPlayer() && ToPlayer()->HasItemFitToSpellRequirements(l_SpellInfo))
                    AddPct(DoneTotalMod, l_Effect->GetAmount());
            }
        }
    }
    ...
    // Agony calculation, 12 ticks
    if (spellProto->Id == 980)
        DoneAdvertisedBenefit = int32(DoneAdvertisedBenefit / 12.0f);
    ...
    if (bonus)
    {
        if (damagetype == DOT)
        {
            ...
            if (bonus->ap_dot_bonus > 0)
            {
                ...
                if (spellProto->Id == 115767 && IsPlayer())
                {
                    if (ToPlayer()->GetSpecializationId() == SPEC_WARRIOR_ARMS)
                        APbonus *= 2.f; //< Deep Wounds Patch 5.2 for Arms Warrior
                    else if (ToPlayer()->GetSpecializationId() == SPEC_WARRIOR_PROTECTION)
                        APbonus /= 2.f; //< According to SimulationCraft
                }
    ...
    if (spellProto->Id == 53301)
    {
        if (auto _effect = victim->GetAuraEffect(53301, EFFECT_2))
            tmpDamage += float(_effect->GetAmount());
    }
...
{
    if (!spellProto || damagetype == DIRECT_DAMAGE || spellProto->Id == 118594)
        return pdamage;
    ...
    // small exception for Echo of Light, Stagger Amount, can't find any general rules
    // Light Stagger, Moderate Stagger and Heavy Stagger ignore reduction mods
    if (spellProto->Id == 77489 || spellProto->Id == 124275 || spellProto->Id == 124274 || spellProto->Id == 124273)
        return pdamage;
    ...
    // Diffuse Magic does not reduce damage from Unstable Affliction when dispelling
    // http://www.mmo-champion.com/threads/1365617-Monk-Diffuse-Magic-Revival-no-longer-a-thing
    if (spellProto->Id == 31117) //< Unstable affliction anti-dispel damage
    {
        if (AuraEffect* l_UnstableAffliction = GetAuraEffect(122783, EFFECT_0)) //< Diffuse Magic
            TakenTotalMod /= CalculatePct(1.0f, 100 + l_UnstableAffliction->GetAmount());
    }
    ...
    switch (spellProto->Id)
    {
        ...
        case 51505:
        case 77451:
            // Additional damage with Flame Shock
            if (AuraEffect* auraEff = GetAuraEffect(8050, EFFECT_2))
                AddPct(TakenTotalMod, auraEff->GetAmount());
...
int32 Unit::SpellBaseDamageBonusDone(SpellSchoolMask schoolMask)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        ...
        int spellPowerPct = GetTotalAuraModifier(SPELL_AURA_MOD_SPELL_POWER_PCT);
...
bool Unit::isSpellCrit(Unit* victim, SpellInfo const* spellProto, SpellSchoolMask schoolMask, WeaponAttackType attackType) const
{
    ...
    //! Fire Elemental (from totem) / Gargoyle can too - but this part is a hack and needs more research
        ...
        case SPELL_DAMAGE_CLASS_MAGIC:
        {
            ...
            if (victim)
            {
                ...
                switch (spellProto->SpellFamilyName)
                {
                    case SPELLFAMILY_GENERIC:
                    {
                        // Unholy Strength - Rune of the Fallen Crusader (53344)
                        if (spellProto->Id == 53365 && (GetTypeId() == TYPEID_PLAYER))
                    ...
                    case SPELLFAMILY_DRUID:
                    {
                        switch (spellProto->Id)
                        {
                            case 8936:  // Regrowth ...
                                ...
                                break;
                            case 6785:  // Ravage
                            case 102545:// Ravage with Incarnation: King of the Jungle
                                // if victim has more than 80% of hp critical chance is increased by 50%
                    ...
                    case SPELLFAMILY_SHAMAN:
                    {
                        switch (spellProto->Id)
                        {
                            ...
                            case 77451: // Lava Burst mastery
                                // Always critical hit, if target doesn't have Roar of Sacrifice
                            ...
                            case 117014:// Elemental Blast
                                // Increase critical chance for 25% if victim has Stormstrike or Stormblast aura
                                if (victim->HasAura(17364, GetGUID()) || victim->HasAura(115356, GetGUID()))
                                    crit_chance += 25.0f;
                                break;
                             default:
                                 break;
        ...
        case SPELL_DAMAGE_CLASS_MELEE:
            if (victim)
            {
                ...
                switch (spellProto->SpellFamilyName)
                {
                    case SPELLFAMILY_DEATHKNIGHT:
                    {
                        // Obliterate and Frost Strike from offhand with Killing Machine.
                        if (spellProto->Id == 66198 || spellProto->Id == 66196)
                        {
                            if (HasAura(51124))
                                return 100.0f;
                        }
...
uint32 Unit::SpellHealingBonusDone(Unit* victim, SpellInfo const* spellProto, uint32 healamount, DamageEffectType damagetype, uint32 stack)
{
    ...
    switch (spellProto->Id)
    {
        case 115714: // Glyph of SpellSteal
        ...
        case 145153: // Dream of Cenarius heal
        case 127626: // Devouring Plague heal
        case 119980: // Conversion
        case 115450: // Detox
        ...
        case 102694:
        case 102695:
            shouldTakeBonus = false;
            break;

        // No bonus/malus for these spells
    ...
    auto applyBonusPct = [shouldTakeBonus](float& p_Var, float p_Pct)
    {
        if (shouldTakeBonus || p_Pct < 0)
            AddPct(p_Var, p_Pct);
    };

    auto addBonusPct = [shouldTakeBonus](int32& p_Var, int32 p_Value)
    {
        if (shouldTakeBonus || p_Value < 0)
            p_Var += p_Value;
    };

    // Healing done percent
    AuraEffectList const& mHealingDonePct = GetAuraEffectsByType(SPELL_AURA_MOD_HEALING_DONE_PERCENT);
    for (AuraEffect const* l_AuraEffect : mHealingDonePct)
        applyBonusPct(DoneTotalMod, l_AuraEffect->GetAmount());

    // Glyph of Healing Storm
    if (getClass() == CLASS_SHAMAN && HasAura(53817) && HasAura(89646))
        if (Aura* maelstromWeapon = GetAura(53817))
            applyBonusPct(DoneTotalMod, maelstromWeapon->GetStackAmount() * 20);

    // done scripted mod (take it from owner)
    Unit* owner = GetOwner() ? GetOwner() : this;
    AuraEffectList const& mOverrideClassScript= owner->GetAuraEffectsByType(SPELL_AURA_OVERRIDE_CLASS_SCRIPTS);
    for (AuraEffect const* auraEffect : mOverrideClassScript)
    {
        if (!auraEffect->IsAffectingSpell(spellProto))
            continue;
        switch (auraEffect->GetMiscValue())
        {
            ...
            case 3736: // Hateful Totem of the Third Wind / Increased Lesser Healing Wave / LK Arena (4/5/6) Totem of the Third Wind / Savage Totem of the Third Wind
                addBonusPct(DoneTotal, auraEffect->GetAmount());
                break;
            case   21: // Test of Faith
            case 6935:
            case 6918:
                if (victim->HealthBelowPct(50))
                    applyBonusPct(DoneTotalMod, auraEffect->GetAmount());
                break;
            default:
                break;
        }
    ...
    // Mastery: Shield Discipline - 77484 , direct heal
    if (healamount > 0 && getClass() == CLASS_PRIEST && HasAura(77484))
    {
        float Mastery = this->GetFloatValue(PLAYER_MASTERY) * 1.6f / 2.0f;
        applyBonusPct(DoneTotalMod, Mastery);
    }
    
    // Mastery : Deep Healing
    if (healamount > 0 && getClass() == CLASS_SHAMAN && HasAura(77226))
    {
        float maximum = GetFloatValue(PLAYER_MASTERY) * 3.0f;
        float healthpct = victim->GetHealthPct();
        float bonus = CalculatePct((100.0f - healthpct), maximum);
        
        applyBonusPct(DoneTotalMod, bonus);
    }
    ...
    {
        float PvPPower = ToPlayer()->GetPvpHealingBonus();
        DoneTotalMod *= PvPPower;
    }

    // Pets ability
    if (spellProto->Id == 90361 && GetOwner() && GetOwner()->ToPlayer() && (victim->GetTypeId() == TYPEID_PLAYER || (victim->GetTypeId() == TYPEID_UNIT && victim->isPet() && victim->GetOwner() && victim->GetOwner()->ToPlayer())))
    {
        float PvPPower = GetOwner()->ToPlayer()->GetPvpHealingBonus();
        DoneTotalMod *= PvPPower;
    }
    ...
    if ((!DoneAdvertisedBenefit || (SpellBaseHealingBonusDone(spellProto->GetSchoolMask()) < SpellBaseDamageBonusDone(spellProto->GetSchoolMask()))) && 
        spellProto->Id != 11958)  // Cold Snap shouldn't take spellpower
    ...
    if (bonus)
    {
        if (damagetype == DOT)
        {
            coeff = bonus->dot_damage;
            if (bonus->ap_dot_bonus > 0)
                addBonusPct(DoneTotal, int32(bonus->ap_dot_bonus * stack * GetTotalAttackPowerValue(
        ...
        else
        {
            coeff = bonus->direct_damage;
            if (bonus->ap_bonus > 0)
                addBonusPct(DoneTotal, bonus->ap_bonus * stack * GetTotalAttackPowerValue(BASE_ATTACK));
        }
    }
    else
    {
        // No bonus healing for SPELL_DAMAGE_CLASS_NONE class spells by default
        if (spellProto->DmgClass == SPELL_DAMAGE_CLASS_NONE && spellProto->Id != 48153)
            return healamount;
    }
    ...
    if (DoneAdvertisedBenefit)
    {
        ...
        addBonusPct(DoneTotal, DoneAdvertisedBenefit * coeff * factorMod);
    }
    ...
    float healModified = heal;
    // apply spellmod to Done amount
    if (Player* modOwner = GetSpellModOwner())
        modOwner->ApplySpellMod(spellProto->Id, damagetype == DOT ? SPELLMOD_DOT : SPELLMOD_DAMAGE, healModified);

    if (shouldTakeBonus || healModified < heal)
        heal = healModified;

    return uint32(std::max(heal, 0.0f));
}

uint32 Unit::SpellHealingBonusTaken(Unit* caster, SpellInfo const* spellProto, uint8 effectIndex, uint32 healamount, DamageEffectType damagetype, uint32 stack)
{
    bool shouldTakeBonus = true;
    switch (spellProto->Id)
    {
        /// No bonus for these spells
        case 34299:  ///< Leader of the Pack (heal)
        case 48503:  ///< Living Seed
        case 53652:  ///< Beacon of Light triggered heal
        case 117895: ///< Eminence (statue)
        case 118594: ///< Void Shift
        case 126890: ///< Eminence
        case 127626: ///< Devouring Plague heal
        case 137562: ///< Nimble Brew
        case 48153:  ///< Guardian Spirit (heal)
            shouldTakeBonus = false;
            break;

        // No bonus/malus for these spells
        case 122281: ///< Healing Elixirs heal
        case 52752:  ///< Ancestral Awakening
            return healamount;
    ...
    for (AuraEffectList::const_iterator i = modHealingPct.begin(); i != modHealingPct.end(); ++i)
        applyBonusPct(TakenTotalMod, (*i)->GetAmount());

    if (auto auraEffect = GetAuraEffect(110310, EFFECT_0))
    {
        if (auto effAmount = auraEffect->GetAmount())
            applyBonusPct(TakenTotalMod, -effAmount);
    }

    // Tenacity increase healing % taken
    if (AuraEffect const* Tenacity = GetAuraEffect(58549, 0))
        applyBonusPct(TakenTotalMod, Tenacity->GetAmount());
    ...
    if (spellProto->Id == 50464)
    {
        ...
        if (HasAura(48438, caster->GetGUID()) ||   // Wild Growth
            ...
            applyBonusPct(TakenTotalMod, 20);
    }

    // Unleashed Fury - Earthliving
    if (auto aura = caster->GetAura(118473))
    {
        auto const& effect = spellProto->Effects[effectIndex];
        bool applyBonus = (spellProto->Id != 1064 &&  // This spell doesn't affect on Chain Heal
            effect.Effect == SPELL_EFFECT_HEAL && (effect.TargetA.GetTarget() == TARGET_UNIT_TARGET_CHAINHEAL_ALLY || effect.TargetA.GetTarget() == TARGET_UNIT_TARGET_ALLY) && effect.TargetB.GetTarget() == 0
            || spellProto->Id == 73921);

        if (applyBonus)
        {
            applyBonusPct(TakenTotalMod, 50);
            caster->RemoveAura(118473);
        }
    }
    
    if (spellProto->Id == 774)
        if (auto effect = caster->GetAuraEffect(138286, EFFECT_0))
            applyBonusPct(TakenTotalMod, effect->GetAmount());

    // Glyph of Animal Bond - 20895
    if (shouldTakeBonus && GetTypeId() == TYPEID_PLAYER && HasAura(20895))
    {
        if (Pet* l_Pet = ToPlayer()->GetPet())
        {
            if (l_Pet->isAlive())
                AddPct(TakenTotalMod, sSpellMgr->GetSpellInfo(24529)->Effects[0].BasePoints);
        }
    }

    // Glyph of Recovery
    if (shouldTakeBonus && caster->getClass() == CLASS_ROGUE && spellProto->Id != 73651 && caster->HasAura(146625) && caster->HasAura(73651))
        healamount = uint32(healamount * 1.2f);
    ...
    if (TakenAdvertisedBenefit)
    {
        ...
        addBonusPct(TakenTotal, TakenAdvertisedBenefit * coeff * factorMod);
    }

    AuraEffectList const& mHealingGet= GetAuraEffectsByType(SPELL_AURA_MOD_HEALING_ABSORB_RECEIVED);
...
bool Unit::IsImmunedToSpell(SpellInfo const* spellInfo, bool permanent)
{
    if (!spellInfo)
        return false;
    ...
    for (uint8 i = 0; i < spellInfo->EffectCount; ++i)
    {
        ...
        if (!spellInfo->Effects[i].IsEffect())
            continue;
        if (!IsImmunedToSpellEffect(spellInfo, i, permanent))
        {
    ...
    // Custom MoP Script for Cyclone, if target has Bladestorm
    if (spellInfo->Id == 33786 && HasAura(46924))
        return true;
...
bool Unit::IsImmunedToSpellEffect(SpellInfo const* spellInfo, uint32 index, bool permanent) const
{
    if (!spellInfo || !spellInfo->Effects[index].IsEffect())
        return false;

    auto l_IsPermanent = [](uint32 p_SpellId) -> bool
    {
        if (p_SpellId == 0)
            return true;

        if (const SpellInfo* l_SpellInfo = sSpellMgr->GetSpellInfo(p_SpellId))
        {
            if (l_SpellInfo->GetMaxDuration() == -1)
                return true;
        }
    ...
    for (SpellImmuneList::const_iterator itr = effectList.begin(); itr != effectList.end(); ++itr)
        if (itr->type == effect)
            return !permanent || l_IsPermanent(itr->spellId);
        ...
        for (AuraEffectList::const_iterator iter = immuneAuraApply.begin(); iter != immuneAuraApply.end(); ++iter)
            if (((*iter)->GetMiscValue() & spellInfo->GetSchoolMask()) &&  // Check school
                ...
                if (!(spellInfo->AttributesEx3 & SPELL_ATTR3_IGNORE_HIT_RESULT))
                    return !permanent || l_IsPermanent((*iter)->GetId());
    }
...
uint32 Unit::MeleeDamageBonusDone(Unit* victim, uint32 pdamage, WeaponAttackType attType, SpellInfo const* spellProto)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER && ToPlayer()->GetSpecializationId() == SPEC_WARRIOR_ARMS && spellProto && spellProto->Id == 50622 && HasAura(46924))
        AddPct(DoneTotalMod, 60);

    if (getClass() == CLASS_ROGUE && HasAura(84601))
        AddPct(DoneTotalMod, 20);

    // Might of the Frozen Wastes - 81333
    // Increase damage dealt by two handed weapons by 30%
    if (pdamage > 0 && GetTypeId() == TYPEID_PLAYER && ToPlayer()->GetSpecializationId() == SPEC_DK_FROST && ToPlayer()->IsTwoHandUsed() && HasAura(81333))
    {
        bool flag = (spellProto && (spellProto->Id == 49020 || spellProto->Id == 49143 || spellProto->Id == 45462 || spellProto->Id == 73975 || spellProto->Id == 49998));// || !spellProto && attType == BASE_ATTACK);
        if (flag)
            AddPct(DoneTotalMod, 30);
    }
    ...
    // Sudden Death - 29725
    if (GetTypeId() == TYPEID_PLAYER && ToPlayer()->GetSpecializationId() == SPEC_WARRIOR_ARMS && (attType == BASE_ATTACK || attType == OFF_ATTACK || spellProto->Id == 76858) && HasAura(29725))
    {
        bool flag = (spellProto && spellProto->Id == 76858 || !spellProto);

        if (flag && roll_chance_i(10))
            CastSpell(this, 52437, true); // Reset Cooldown of Colossus Smash
    }

    // Custom MoP Script
    // 76659 - Mastery : Wild Quiver
    if (GetTypeId() == TYPEID_PLAYER && attType == RANGED_ATTACK && (spellProto->Id != 76663) && HasAura(76659))
    {
        float Mastery = GetFloatValue(PLAYER_MASTERY) * 2.0f;

        // If it procs from Barrage , we should reduce proc chance x5
        if (spellProto->Id == 120361)
            Mastery /= 5.0f;
    ...
    else if (GetTypeId() == TYPEID_PLAYER && HasAura(77219) && !HasAura(103958))
    {
        float Mastery = GetFloatValue(PLAYER_MASTERY);
        AddPct(DoneTotalMod, Mastery);
    }
    ...
    {
        Unit* owner = GetOwner();
        if (owner && owner->GetTypeId() == TYPEID_PLAYER && owner->HasAura(77219))
        {
            float Mastery = owner->GetFloatValue(PLAYER_MASTERY);
            AddPct(DoneTotalMod, Mastery);
        }
    ...
    if (GetTypeId() == TYPEID_PLAYER && victim && pdamage != 0 && attType == BASE_ATTACK && (!spellProto || spellProto->Id == 121473))
    {
        if (HasAura(76806))
        {
            float Mastery = GetFloatValue(PLAYER_MASTERY) * 2.0f;
    ...
    if (GetTypeId() == TYPEID_PLAYER && victim && pdamage != 0 && spellProto && spellProto->Id != 76858 && HasAura(144436) && victim->HasAura(113746))
        this->CastSpell(this, 144438, true);

    // Custom MoP Script
    // 76613 - Mastery : Icicles for Water elemental Melee damage
    if (GetTypeId() == TYPEID_PLAYER && victim && pdamage != 0)
    {
        if (isPet())
        {
            Unit* owner = this->GetOwner();
            if (owner->GetTypeId() == TYPEID_PLAYER && owner->HasAura(76613))
            {
                float Mastery = owner->GetFloatValue(PLAYER_MASTERY) * 2.0f;
                AddPct(DoneTotalMod, Mastery);
            }
...
void Unit::Mount(uint32 mount, uint32 VehicleId, uint32 creatureEntry, bool isTaxi)
{
    ...
    if (Player* player = ToPlayer())
    {
        ...
        if ((creatureEntry == 304 || creatureEntry == 14505 || creatureEntry == 73965 || creatureEntry == 73966) && player->HasAura(56232))
...
void Unit::Dismount()
{
    ...
    WorldPacket data(SMSG_DISMOUNT, 1 + 8);
...
void Unit::UpdateMount()
{
    ...
    if (mountType)
    {
        ...
        GetZoneAndAreaId(zoneId, areaId, true);
    ...
    if (_mount != newMount)
    {
        ...
        if (newFlyer)
        {
            SpellInfo const* newSpellInfo = sSpellMgr->GetSpellInfo(newSpell);
            if (newSpellInfo)
            {
                if (!newSpellInfo->HasAura(SPELL_AURA_MOD_INCREASE_MOUNTED_FLIGHT_SPEED))
                    newFlyer = false;
            }
        }
        ...
        if (oldSpell != newSpell)
        {
            ...
            if (Player* player = ToPlayer())
            {
                ...
                {
                    if (oldSpell)
                        pet->RemoveAurasDueToSpell(oldSpell);

                    if (newSpell)
                        pet->CastSpell(pet, newSpell, true, NULL, effect);
                }
            }
        }
...
void Unit::SetInCombatState(bool PvP, Unit* enemy, bool isControlled)
{
    ...
    if (HasUnitState(UNIT_STATE_EVADE) && GetDisplayId() != 42533)
        return;
    ...
    if (Creature* creature = ToCreature())
    {
        ...
        if ((IsAIEnabled && creature && creature->AI()->IsEscorted()) ||
...
int32 Unit::ModifyHealth(int32 dVal)
{
    ...
    if (dVal < 0)
    {
        val = (curHealth + dVal) < 0 ? 0 : curHealth + dVal;
    }
    else
    {
        val = curHealth + dVal;
    ...
    {
        SetHealth(val);
        gain = (int32)val - (int32)curHealth;
    }
    else if (curHealth != maxHealth)
    {
        SetHealth(maxHealth);
        gain = (int32)maxHealth - (int32)curHealth;
    }
...
int32 Unit::ModifyPower(Powers power, int32 dVal, SpellInfo const* p_SpellInfo /*= nullptr*/)
{
    int32 gain = 0;
    ...
    if (GetTypeId() == TYPEID_PLAYER)
        sScriptMgr->OnModifyPower(this->ToPlayer(), power, dVal, p_SpellInfo);
...
void Unit::UpdateSpeed(UnitMoveType mtype, bool forced)
{
    ...
    switch (mtype)
    {
        ...
        case MOVE_RUN:
        {
            if (IsMounted() || (isPet() && GetOwner() && GetOwner()->IsMounted())) // Use on mount auras
            {
    ...
    // Hack fix for Frost Pillar with Glyph of Pillar of Frost
    if (ToPlayer() && getClass() == CLASS_DEATH_KNIGHT && ToPlayer()->GetSpecializationId() == SPEC_DK_FROST)
    {
        if (HasAura(51271) && HasAura(58635))
            slow = -70;
    }
...
void Unit::SetSpeed(UnitMoveType mtype, float rate, bool forced)
{
    ...
    WorldPacket data(!forced ? moveTypeToOpcode[mtype][0] : moveTypeToOpcode[mtype][1], 12);
    WriteMovementInfo(data, &extra);
    SendMessageToSet(&data, true);
}
...
void Unit::TauntFadeOut(Unit* taunter)
{
    ...
    if (m_ThreatManager.isThreatListEmpty() && GetDisplayId() != 42533)
    {
        if (creature->IsAIEnabled)
...
bool Creature::IsShouldKeepOrientationWhileCasting() const
{
    // We need to find spells that requires no target
    // Aoe, cone and self-casted spells

    if (Spell const* channelSpell = GetCurrentSpell(CurrentSpellTypes::CURRENT_CHANNELED_SPELL))
    {
        // while needed channel spell is preparing then targetMask=0 and targetGuid=0
        // while needed channel spell is casting then targetMask=2 and targetGuid=0
        uint32 targetMask = channelSpell->m_targets.GetTargetMask();
        uint32 targetGuid = channelSpell->m_targets.GetObjectTargetGUID();
        bool noTargets = (targetMask == 0) || (targetGuid == 0);
        if (noTargets)
            return true;
    }

    if (Spell const* genericSpell = GetCurrentSpell(CurrentSpellTypes::CURRENT_GENERIC_SPELL))
    {
        // do the same
        uint32 targetMask = genericSpell->m_targets.GetTargetMask();
        uint32 targetGuid = genericSpell->m_targets.GetObjectTargetGUID();
        bool noTargets = (targetMask == 0) || (targetGuid == 0);
        if (noTargets)
            return true;
    }
...
int32 Unit::CalculateSpellDamage(Unit const* target, SpellInfo const* spellProto, uint8 effect_index, int32 const* basePoints, int32 itemLevel /*= -1*/) const
{
    return spellProto->Effects[effect_index].CalcValue(this, basePoints, target, nullptr, itemLevel);
}
...
void Unit::ModSpellCastTime(SpellInfo const* spellProto, int32 & castTime, Spell* spell)
{
    ...
    if (spellProto->Id == 138724)
        castTime = 0;
    
    if (spellProto->HasEffect(SPELL_EFFECT_SKINNING))
        if (auto effect = GetAuraEffect(68978, EFFECT_1))
            AddPct(castTime, -effect->GetAmount());

    if (HasAura(126342))
    {
        static std::array<SpellEffects, 2> s_Effects = { SPELL_EFFECT_SKINNING, SPELL_EFFECT_OPEN_LOCK };
        for (auto effect : s_Effects)
        {
            if (!spellProto->HasEffect(effect))
                continue;

            if (effect == SPELL_EFFECT_OPEN_LOCK && !spellProto->HasEffect(SPELL_EFFECT_SKILL))
                continue;
    ...
    // Custom FUN - Make glyphs/enchants instant spells.
    if (sWorld->getBoolConfig(CONFIG_INSTANT_GLYPH_ENCHANT))
    {
        if ((spellProto->HasEffect(SPELL_EFFECT_ENCHANT_ITEM) ||            // Enchants
            ...
            spellProto->HasEffect(SPELL_EFFECT_LEARN_SPELL)  ||             // Learn Glyphs
            spellProto->HasEffect(SPELL_EFFECT_CREATE_ITEM))  ||            // Create items (professions)
...
{
    for (DiminishingReturn& dimReturn : m_Diminishing)
...
bool Unit::CanIgnoreMinRange(Unit const* target, SpellInfo const* spellInfo) const
{
    if (!target)
        return false;

    // hunter can hit some mobs with huge model
...
bool Unit::SetAndHandleModifierValue(UnitMods unitMod, UnitModifierType modifierType, float value)
{
    if (unitMod >= UNIT_MOD_END || modifierType >= MODIFIER_TYPE_END)
        return false;
...
uint32 Unit::GetPowerIndexByClass(uint32 p_PowerID, uint32 p_ClassID) const
{
    ...
    if (GetTypeId() != TypeID::TYPEID_PLAYER)
    {
        ...
        if (l_DisplayPower == (Powers)p_PowerID)
            l_PowerIndex = 0;
        else if (p_PowerID == Powers::POWER_ALTERNATE_POWER)
            l_PowerIndex = 1;
        else
    ...
    ChrClassesEntry const* l_ClassEntry = sChrClassesStore.LookupEntry(p_ClassID);
    ...
    for (uint32 l_I = 0; l_I <= sChrPowerTypesStore.GetNumRows(); ++l_I)
    {
        ...
        if (l_PowerEntry->classId != p_ClassID)
            continue;

        if (l_PowerEntry->power == p_PowerID)
            return l_PowerIndex;
...
void Unit::SetPower(Powers power, int32 val, bool regen)
{
    ...
    if (IsInWorld() && (!regen || regen_diff > 2000))
        SendPowerUpdate(power, val);

    // Custom MoP Script
    // Item - Warlock T16 4P Bonus - 145091
    if (power == POWER_BURNING_EMBERS)
    {
        if (val == maxPower && ToPlayer()->HasAura(145091) && !ToPlayer()->HasSpellCooldown(145165))
        {
            ToPlayer()->AddAura(145164, ToPlayer());
            ToPlayer()->AddSpellCooldown(145165, 0, 10 * IN_MILLISECONDS);
        }
        
        if (val <= 1 && HasAura(108683)) // Fire and brimstone
            RemoveAura(108683);
    }
    ...
    if (Player* player = ToPlayer())
    {
        if (power != POWER_SOUL_SHARDS && power != POWER_ECLIPSE && power != POWER_HOLY_POWER && 
            player->HaveSpectators())
        {
            uint8 mod = 1;
...
void Unit::SendPowerUpdate(Powers power)
{
    SendPowerUpdate(power, GetPower(power));
}

void Unit::SendPowerUpdate(Powers power, int32 val)
{
    ...
    switch (power)
    {
        ...
        case POWER_ENERGY:
        {
            if (HasUnitTypeMask(UNIT_MASK_MINION))
                if (Minion const* minion = (Minion const*)this)
                    return minion->IsWarlockPet() ? 200 : 100;
        ...
        case POWER_SHADOW_ORB:
            return (GetTypeId() == TYPEID_PLAYER && ToPlayer()->getClass() == CLASS_PRIEST && (ToPlayer()->GetSpecializationId() == SPEC_PRIEST_SHADOW) ? 3 : 0);
        case POWER_BURNING_EMBERS:
            return (GetTypeId() == TYPEID_PLAYER && ToPlayer()->getClass() == CLASS_WARLOCK && (ToPlayer()->GetSpecializationId() == SPEC_WARLOCK_DESTRUCTION) ? 40 : 0);
        case POWER_DEMONIC_FURY:
            return (GetTypeId() == TYPEID_PLAYER && ToPlayer()->getClass() == CLASS_WARLOCK && (ToPlayer()->GetSpecializationId() == SPEC_WARLOCK_DEMONOLOGY) ? 1000 : 0);
        case POWER_SOUL_SHARDS:
            return (GetTypeId() == TYPEID_PLAYER && ToPlayer()->getClass() == CLASS_WARLOCK && (ToPlayer()->GetSpecializationId() == SPEC_WARLOCK_AFFLICTION) ? 400 : 0);
        case POWER_ECLIPSE:
            return (GetTypeId() == TYPEID_PLAYER && ToPlayer()->getClass() == CLASS_DRUID && (ToPlayer()->GetSpecializationId() == SPEC_DRUID_BALANCE) ? 100 : 0); // Should be -100 to 100 this needs the power to be int32 instead of uint32
...
void CharmInfo::InitPetActionBar()
{
    ...
    SetActionBar(ACTION_BAR_INDEX_PET_SPELL_END, REACT_HELPER, ACT_REACTION);
...
void CharmInfo::LoadPetActionBar(const std::string& data)
{
    ...
    for (; index < ACTION_BAR_INDEX_END; ++iter, ++index)
    {
        ...
        // skip aggressive  stance for pet
        if (type == ACT_COMMAND && action == REACT_AGGRESSIVE)
            continue;
...
bool InitTriggerAuraData()
{
    ...
    isTriggerAura[SPELL_AURA_MOD_ATTACKSPEED_2] = true;
...
void Unit::ProcDamageAndSpellFor(bool isVictim, Unit* target, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, SpellInfo const* procSpell, uint32 damage, uint32 absorb, SpellInfo const* procAura, Aura const* procEffect, Spell const* spell)
{
    ...
    if (procFlag & MELEE_BASED_TRIGGER_MASK && target)
    {
        ...
        if (procExtra & (PROC_EX_CRITICAL_HIT|PROC_EX_PARRY|PROC_EX_DODGE|PROC_EX_BLOCK))
        {
            ...
            else // For attacker
            {
                ...
                if (procExtra & PROC_EX_DODGE && GetTypeId() == TYPEID_PLAYER && getClass() == CLASS_WARRIOR && this->ToPlayer()->GetSpecializationId() == SPEC_WARRIOR_ARMS)
                {
    ...
    // Dematerialize
    if (target && target->GetTypeId() == TYPEID_PLAYER && procSpell && (procSpell->GetAllEffectsMechanicMask() & (1 << MECHANIC_STUN)) && target->HasAura(122464))
    {
    ...
    if (GetTypeId() == TYPEID_PLAYER && target && procSpell && procSpell->Id == 1752 && target->HasAura(84617, GetGUID()))
        if (roll_chance_i(20))
            ToPlayer()->AddComboPoints(target, 1);

    // Bandit's Guile - 84654
    // Your Sinister Strike and Revealing Strike abilities increase your damage dealt by up to 30%
    if (!isVictim && GetTypeId() == TYPEID_PLAYER && procSpell && (procSpell->Id == 84617 || procSpell->Id == 1752) && HasAura(84654))
    {
        insightCount++;
    ...
    if (GetTypeId() == TYPEID_PLAYER && procSpell && procSpell->Id != 108839 && HasAura(108839) &&
        ...
        if (Aura* aura = GetAura(108839, GetGUID()))
        {
            if (aura->GetStackAmount() - 1 > 0)
                aura->SetStackAmount(aura->GetStackAmount() - 1);
            else
                aura->DropCharge();
        }

    if (GetTypeId() == TypeID::TYPEID_UNIT && isHunterPet() && GetOwner() && damage > 0 && procSpell)
    {
        switch (procSpell->Id)
        {
            /// Only triggers on Basic Attacks
            case 16827: ///< Claw
            case 17253: ///< Bite
            case 49966: ///< Smack
            {
                if (Player* l_Owner = GetOwner()->ToPlayer())
                {
                    if (Pet* l_Pet = ToPet())
                    {
                        if (l_Pet->IsPermanentPetFor(l_Owner))
                        {
                            /// Hack Fix Frenzy
                            if (l_Owner->HasAura(19623) && roll_chance_i(40))
    ...
    if (!isVictim && GetTypeId() == TYPEID_PLAYER && getClass() == CLASS_DEATH_KNIGHT && procSpell && HasAura(51124))
    {
        if (haveOffhandWeapon())
        {
            if (procSpell->Id == 66198 || procSpell->Id == 66196)
                RemoveAura(51124);
        }
        else if (procSpell->Id == 49020 || procSpell->Id == 49143)
            RemoveAura(51124);
    }

    // Fix Drop charge for Blindsight
    if (GetTypeId() == TYPEID_PLAYER && getClass() == CLASS_ROGUE && procSpell && procSpell->Id == 111240 && HasAura(121152))
        RemoveAura(121153);

    // Fix Drop charge for Fingers of Frost
    if (!isVictim && GetTypeId() == TYPEID_PLAYER && getClass() == CLASS_MAGE && procSpell && (procSpell->Id == 30455 || procSpell->Id == 44572))
    {
    ...
    ProcEventInfo eventInfo = ProcEventInfo(actor, actionTarget, target, procFlag, 0, 0, procExtra, spell, &damageInfo, &healInfo);
    ...
    for (AuraApplicationMap::const_iterator itr = GetAppliedAuras().begin(); itr!= GetAppliedAuras().end(); ++itr)
    {
        ...
        ProcTriggeredData triggerData(_base);
        ...
        SpellInfo const* spellProto = _base->GetSpellInfo();
        if (!spellProto)
            continue;
        ...
        if ((damage || absorb) && !isVictim && GetTypeId() == TYPEID_PLAYER)
            active = true;
        ...
        if (procExtra & PROC_EX_NORMAL_HIT)
        {
            if (itr->first == 77606)
            {
                if (procSpell && procSpell->IsCanBeStolen())
                    active = true;
            }
        }
        ...
        if (triggerData.aura)
        {
            SpellInfo const* m_tempInfo = triggerData.aura->GetSpellInfo();

            if (m_tempInfo && (m_tempInfo->Id == 33757 ||
            m_tempInfo->Id == 28305 || m_tempInfo->Id == 2823 ||
            m_tempInfo->Id == 3408 || m_tempInfo->Id == 5761 ||
            m_tempInfo->Id == 8679 || m_tempInfo->Id == 108211 ||
            m_tempInfo->Id == 108215 || m_tempInfo->Id == 51530 || m_tempInfo->GetSpellSpecific() == SPELL_SPECIFIC_SEAL ||
            m_tempInfo->HasAura(SPELL_AURA_MOD_STEALTH) || m_tempInfo->HasAura(SPELL_AURA_MOD_INVISIBILITY)))
            active = true;
        }

        bool custom = (spellProto->Id == 19387 && procSpell && (procSpell->Id == 13810)) || (spellProto->Id == 114108 && (procFlag & PROC_FLAG_DONE_SPELL_MAGIC_DMG_CLASS_NEG));

        if (!custom && !IsTriggeredAtSpellProcEvent(target, triggerData.aura, procSpell, procFlag, procExtra, attType, isVictim, active, triggerData.spellProcEvent))
            continue;
        ...
        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        {
            ...
            {
                AuraEffect* aurEff = _base->GetEffect(i);
                // Skip this auras
    ...
    for (ProcTriggeredList::const_iterator i = procTriggered.begin(); i != procTriggered.end(); ++i)
    {
        ...
        if (GetTypeId() == TYPEID_PLAYER)
        {
            ...
            if (cooldown && ToPlayer()->HasSpellCooldown(spellInfo->Id) && spellInfo->Id != 16689 && spellInfo->Id != 74001)
                continue;
        }

        i->aura->SetLastProcSuccessTime(now);

        // Fix Improved Lightning Shield
        if (spellInfo->Id == 324 && HasAura(100956))
        ...
        if (isVictim && spellInfo->HasAura(SPELL_AURA_MOD_STEALTH) && HasAura(131369))
            useCharges = false;
        ...
        if (!handled)
        {
            for (uint8 effIndex = 0; effIndex < MAX_SPELL_EFFECTS; ++effIndex)
            {
                ...
                switch (triggeredByAura->GetAuraType())
                {
                    ...
                    case SPELL_AURA_OVERRIDE_AUTOATTACK_WITH_MELEE_SPELL:
                    {
                        bool canProc = true;
                        if (target && triggeredByAura->GetSpellInfo()->HasCustomAttribute(SPELL_ATTR0_CU_PVE_PROC_ONLY))
                            canProc = !target->ToPlayer();

                        // Don`t drop charge or add cooldown for not started trigger
                        if (canProc && HandleProcTriggerSpell(target, damage, absorb, triggeredByAura, procSpell, procFlag, procExtra, applyCooldown))
                            takeCharges = true;
                        break;
                    }
                    ...
                    case SPELL_AURA_PROC_TRIGGER_SPELL_COPY:
                    {
                        if (isVictim && procSpell && procSpell->Id == 108853 && triggeredByAura->GetId() == 44448)
                            break;
                        if (HandleDummyAuraProc(target, damage, absorb, triggeredByAura, procSpell, procFlag, procExtra, procEffect))
                            takeCharges = true;
                        break;
                    }
                    case SPELL_AURA_PROC_ON_POWER_AMOUNT:
                    {
                        if (HandleAuraProcOnPowerAmount(target, damage, triggeredByAura, procSpell, procFlag, procExtra))
                            takeCharges = true;
                        break;
                    }
                    ...
                    {
                        if (HandleProcTriggerSpell(target, damage, absorb, triggeredByAura, procSpell, procFlag, procExtra, applyCooldown))
                            takeCharges = true;
                        break;
                    }
                    ...
                    case SPELL_AURA_MOD_POWER_COST_SCHOOL:
                        ...
                        // School Check
                        if (!(triggeredByAura->GetMiscValue() & procSpell->SchoolMask))
                            break;

                        for (auto itr : procSpell->SpellPowers)
                        {
                            if ((itr->Cost != 0 || itr->CostBasePercentage != 0))// Cost check
                            {
                                takeCharges = true;
                                break;
                            }
                    ...
                    default:
                        ...
                        if (triggeredByAura->GetId() == 974 || triggeredByAura->GetId() == 146953)
                            break;
        ...
        if (i->aura->GetId() == 105421 && procSpell && procSpell->Id == 31803)
            takeCharges = false;

        if (i->aura->GetId() == 119032)
        {
            const static AuraType s_CCAuras[] = { SPELL_AURA_MOD_CONFUSE, SPELL_AURA_MOD_FEAR, SPELL_AURA_MOD_FEAR_2, SPELL_AURA_MOD_STUN, SPELL_AURA_MOD_ROOT, SPELL_AURA_TRANSFORM };
            for (AuraType ccAura : s_CCAuras)
            {
                if (HasAuraType(ccAura))
                {
                    takeCharges = false;
                    break;
                }
        ...
        if (takeCharges && cooldown && ToPlayer() && applyCooldown)
            ToPlayer()->AddSpellCooldown(spellInfo->Id == 974 ? 379 : spellInfo->Id, 0, cooldown);
    }
...
{
    // CC don't breaks on positive procs (heals, etc.)
    if (procFlag & POSITIVE_PROC_FLAG_MASK)
        return true;
    
    if (spellInfo->Id == 19503 && procSpell && procSpell->Id == 76663)
        return true;
    ...
    if (procSpell && procSpell->Id == 117640)
        return true;

    // Dragon's Breath shouldn't break on DoT
    if (procSpell && procSpell->Id == 31661 && procExtra & PROC_EX_INTERNAL_DOT)
        return true;
...
void Unit::GetProcAurasTriggeredOnEvent(std::list<AuraApplication*>& aurasTriggeringProc, std::list<AuraApplication*>* procAuras, ProcEventInfo eventInfo)
{
    ...
    if (procAuras)
    {
        for (std::list<AuraApplication*>::iterator itr = procAuras->begin(); itr!= procAuras->end(); ++itr)
        {
            ...
            {
                auto _base = (*itr)->GetBase();
                if (_base->IsProcTriggeredOnEvent(*itr, eventInfo))
                {
                    _base->PrepareProcToTrigger(*itr, eventInfo);
    ...
    else
    {
        ...
        {
            auto _base = itr->second->GetBase();
            if (_base->IsProcTriggeredOnEvent(itr->second, eventInfo))
            {
                _base->PrepareProcToTrigger(itr->second, eventInfo);
...
void Unit::SendPetTalk(uint32 pettalk)
{
    ...
    uint8 bitsOrder[8] = { 0, 4, 6, 7, 2, 1, 5, 3 };
...
void Unit::StopMoving()
{
    ...
    if (!IsInWorld() || movespline->Finalized() || !movespline->Initialized())
        return;
...
void Unit::SetDisplayId(uint32 modelId, bool removeCloneAura)
{
    if (removeCloneAura)
...
void Unit::ClearComboPointHolders()
{
    while (!m_ComboPointHolders.empty())
    {
        ...
        Player* player = ObjectAccessor::FindPlayer(MAKE_NEW_GUID(lowguid, 0, HighGuid::Player));
...
Unit* Unit::SelectNearbyTarget(Unit* exclude, float dist, bool checkValidAttack, SpellInfo const* spellInfo) const
{
    ...
    {
        if (!IsWithinLOSInMap(*tIter) || (*tIter)->isTotem() || (*tIter)->isSpiritService() || (*tIter)->GetCreatureType() == CREATURE_TYPE_CRITTER || (checkValidAttack && !_IsValidAttackTarget(*tIter, spellInfo)))
            targets.erase(tIter++);
        else
            ++tIter;
    }
...
Unit* Unit::SelectNearbyAlly(Unit* exclude, float dist, bool checkValidAssist, SpellInfo const* spellInfo) const
{
    ...
    for (std::list<Unit*>::iterator tIter = targets.begin(); tIter != targets.end();)
    {
        if (!IsWithinLOSInMap(*tIter) || (*tIter)->isTotem() || (*tIter)->isSpiritService() || (*tIter)->GetCreatureType() == CREATURE_TYPE_CRITTER || (checkValidAssist && !_IsValidAssistTarget(*tIter, spellInfo)))
            targets.erase(tIter++);
        else
            ++tIter;
    }
...
bool Unit::IsTriggeredAtSpellProcEvent(Unit* victim, Aura* aura, SpellInfo const* procSpell, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, bool isVictim, bool active, SpellProcEventEntry const* & spellProcEvent)
{
    ...
    {
        if (!(spellInfo->AttributesEx3 & SPELL_ATTR3_CAN_PROC_WITH_TRIGGERED))
            return false;
    }

    // Check spellProcEvent data requirements
    if (!sSpellMgr->IsSpellProcEventCanTriggeredBy(spellProcEvent, EventProcFlag, procSpell, spellInfo, procFlag, procExtra, active))
    {
        // Hack Fix Backdraft can be triggered if damage are absorbed
        if (spellInfo && spellInfo->Id == 117896 && procSpell && procSpell->Id == 17962 && procExtra && (procExtra & PROC_EX_ABSORB))
            return true;
        else if (spellInfo && spellInfo->Id == 44448 && procSpell &&
        ...
        else if (spellInfo && spellInfo->Id == 121152 && procSpell && procSpell->Id == 1329)
            return true;
        else
            return false;
    }
    ...
    if (!isVictim && GetTypeId() == TYPEID_PLAYER)
    {
        ...
        if (spellInfo->EquippedItemClass == ITEM_CLASS_WEAPON)
        {
            ...
            if (attType == OFF_ATTACK || (!item && getClass() == CLASS_SHAMAN))
...
bool Unit::HandleAuraRaidProcFromChargeWithValue(AuraEffect* triggeredByAura)
{
    ...
    auto _base = triggeredByAura->GetBase();

    // jumps
    int32 jumps = _base->GetCharges()-1;

    // current aura expire
    _base->SetCharges(1);             // will removed at next charges decrease
    ...
    if (jumps > 0)
    {
        if (Unit* caster = triggeredByAura->GetCaster())
        {
            ...
            if (jumps == 3)
            {
                if (AuraEffect* glyphEffect = caster->GetAuraEffect(55685, EFFECT_0)) //< Glyph of Prayer of Mending
                    bonus = glyphEffect->GetAmount();
            }
        }
    }

    // heal
    CastCustomSpell(this, 33110, &heal, &bonus, NULL, true, NULL, nullptr, caster_guid);
    return true;
...
void Unit::Kill(Unit* victim, bool durabilityLoss, SpellInfo const* spellProto)
{
    ...
    Player* player = GetCharmerOrOwnerPlayerOrPlayerItself();
    Creature* l_KilledCreature = victim->ToCreature();

    bool isRewardAllowed = true;
    if (l_KilledCreature)
    {
        isRewardAllowed = l_KilledCreature->IsDamageEnoughForLootingAndReward();
        if (!isRewardAllowed)
            l_KilledCreature->SetLootRecipient(NULL);
    }

    if (isRewardAllowed && l_KilledCreature && l_KilledCreature->GetLootRecipient())
        player = l_KilledCreature->GetLootRecipient();
    ...
    if (isRewardAllowed && player && player != victim)
    {
        ...
        WorldPacket data(SMSG_PARTY_KILL_LOG, 1 + 8);
        ...
        if (Group* group = player->GetGroup())
        {
            ...
            if (l_KilledCreature)
            {
                group->UpdateLooterGuid(l_KilledCreature, true);
                if (group->GetLooterGuid())
                {
                    looter = ObjectAccessor::FindPlayer(group->GetLooterGuid());
                    if (looter)
                    {
                        l_KilledCreature->SetLootRecipient(looter);   // update creature loot recipient to the allowed looter.
                        group->SendLooter(l_KilledCreature, looter);
                    }
                    else
                        group->SendLooter(l_KilledCreature, NULL);
                }
                else
                    group->SendLooter(l_KilledCreature, NULL);

                group->UpdateLooterGuid(l_KilledCreature);
            }
        }
        else
        {
            ...
            if (l_KilledCreature)
            {
                ObjectGuid creatureGuid = l_KilledCreature->GetGUID();

                data.Initialize(SMSG_LOOT_LIST);
        ...
        if (l_KilledCreature)
        {
            Loot* loot = &l_KilledCreature->loot;
            if (l_KilledCreature->lootForPickPocketed)
                l_KilledCreature->lootForPickPocketed = false;

            loot->clear();
            if (uint32 lootid = l_KilledCreature->GetCreatureTemplate()->lootid)
                loot->FillLoot(lootid, LootTemplates_Creature, looter, false, false, l_KilledCreature->GetLootMode());

            uint32 mingold = l_KilledCreature->GetCreatureTemplate()->mingold;
            uint32 maxgold = l_KilledCreature->GetCreatureTemplate()->maxgold;

            if (l_KilledCreature->GetMap()->IsDungeon())
            {
                if (InstanceMap* instance = l_KilledCreature->GetMap()->ToInstanceMap())
                {
            ...
            loot->generateMoneyLoot(mingold, maxgold);
        }
    ...
    else                                                // creature died
    {
        if (!l_KilledCreature->isPet())
        {
            ...
            CreatureTemplate const* cInfo = l_KilledCreature->GetCreatureTemplate();
        ...
        if (TempSummon* summon = l_KilledCreature->ToTempSummon())
            if (Unit* summoner = summon->GetSummoner())
                if (summoner->ToCreature() && summoner->IsAIEnabled)
                    summoner->ToCreature()->AI()->SummonedCreatureDies(l_KilledCreature, this);
        ...
        if (l_KilledCreature->GetInstanceId())
        {
            ...
            if (l_InstanceMap->IsRaid() && l_CreditedPlayer)
            {
                if (l_KilledCreature->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_INSTANCE_BIND)
                    ((InstanceMap*)l_InstanceMap)->PermBindAllPlayersForRaid(l_CreditedPlayer);
            }
            else if (l_InstanceMap->IsRaidOrHeroicDungeon() && l_CreditedPlayer)
            {
                if (l_KilledCreature->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_INSTANCE_BIND)
                    ((InstanceMap*)l_InstanceMap)->PermBindAllPlayers(l_CreditedPlayer);
            }
            else if (l_InstanceMap->IsChallengeMode() && l_CreditedPlayer)
            {
            ...
            else
            {
                ...
                time_t resettime = l_KilledCreature->GetRespawnTimeEx() + 2 * HOUR;
                if (InstanceSave* save = sInstanceSaveMgr->GetInstanceSave(l_KilledCreature->GetInstanceId()))
...
/// Blizzard Loss of Control Alert System
void Unit::SendLossOfControlAuraUpdate(AuraApplication const* p_AurApp, Mechanics p_Mechanic, SpellEffIndex p_EffIndex, LossOfControlType p_Type)
{
    ...
    WorldPacket l_Data(Opcodes::SMSG_LOSS_OF_CONTROL_AURA_UPDATE);
    l_Data.WriteBits(1, 22);
    l_Data.WriteBits(p_Type, 8);
    l_Data.WriteBits(p_Mechanic, 8);
...
void Unit::SetRooted(bool apply)
{
    if (apply)
    {
        ...
        PacketSender(this, SMSG_SPLINE_MOVE_ROOT, SMSG_MOVE_ROOT, false).Send();
    }
    else
    {
        if (!HasUnitState(UNIT_STATE_STUNNED))      // prevent moving if it also has stun effect
        {
            PacketSender(this, SMSG_SPLINE_MOVE_UNROOT, SMSG_MOVE_UNROOT, false).Send();
            RemoveUnitMovementFlag(MOVEMENTFLAG_ROOT);
        }
    }
}
...
void Unit::SendPlaySpellVisualKit(uint32 id, uint32 unkParam)
{
    ...
    uint8 bitOrder[8] = { 2, 5, 7, 3, 4, 6, 0, 1 };
...
void Unit::SendPlaySpellVisual(uint32 id, Unit* target, float travelSpeed, bool thisAsPos /*= false*/, bool speedAsTime /*= false*/)
{
    ...
    WorldPacket data(SMSG_PLAY_SPELL_VISUAL, 4 + 4 + 2 + 4 + 2 + 4 + 4 + 3 + 8 + 8);

    data << uint32(id);
    data << float(l_Pos.m_positionZ);
    data << uint16(0);            // missReason/reflectStatus
    data << float(l_Pos.m_positionY);
    data << uint16(0);            // missReason/reflectStatus
    data << float(l_Pos.m_positionX);
    data << float(travelSpeed);
...
void Unit::ApplyResilience(Unit const* victim, int32* damage) const
{
    ...
    // Hack for Amber-Shaper Un'sok, Heart of Fear
    if (GetMapId() == 1009)
    {
        if (HasAura(122370) || target->HasAura(122370))
            return;
    }
    ...
    static void UpdateVisibility(Unit* me)
     {
        ...
        float l_MinDistanceSQ = g_RequiredMoveDistanceSq[l_Map->GetEntry()->map_type];
...
void Unit::SendMoveKnockBack(Player* player, float speedXY, float speedZ, float vcos, float vsin)
{
    ...
    m_movementInfo.jump.cosAngle = vcos;
    m_movementInfo.jump.sinAngle = vsin;
    m_movementInfo.jump.xyspeed = speedXY;
    m_movementInfo.jump.zspeed = speedZ;
...
void Unit::KnockbackFrom(float x, float y, float speedXY, float speedZ, bool useObjectRotation)
{
    Player* player = NULL;
    if (GetTypeId() == TYPEID_PLAYER)
        player = ToPlayer();
    ...
    else
    {
        ...
        {
            float orientation = !useObjectRotation ? GetAngle(x, y) : GetOrientation();
            float _x, _y, _z;
            float moveTimeHalf = -speedZ / Movement::gravity;
            float dist = 2 * moveTimeHalf * speedXY;
            GetNearPoint(this, _x, _y, _z, GetObjectSize(), dist, orientation);

            UpdatePosition(_x, _y, _z, orientation);
            AddUnitState(UNIT_STATE_RECAL_CONFUSE);
        }
...
bool Unit::HandleSpellClick(Unit* clicker, int8 seatId)
{
    ...
    for (SpellClickInfoContainer::const_iterator itr = clickPair.first; itr != clickPair.second; ++itr)
    {
        ...
        if (!spellEntry)
        {
            sLog->OutTrinity("HandleSpellClick: spellEntry pointer is NULL!!");
            return false;
        }
...
void Unit::_ExitVehicle(Position const* exitPosition)
{
    ...
    {
        WorldPacket data(SMSG_SPLINE_MOVE_UNROOT, 30);
...
void Unit::NearTeleportTo(Position pos, bool casting /* = false */)
{
    ...
    NearTeleportTo(x, y, z, o, casting);
}

void Unit::SendTeleportPacket(Position &newPos)
{
    ...
    data << float(newPos.m_positionX);
    data << float(newPos.m_positionZ);
    data << float(newPos.m_positionY);
    data << uint32(0);                  //  mask ? 0x180 on retail sniff
    data << float(newPos.m_orientation);
...
void Unit::SendThreatListUpdate()
{
    if (!getThreatManager().isThreatListEmpty())
    {
        ...
        WorldPacket data(SMSG_THREAT_UPDATE, 3 + 1 + 8 + (count * 13));
        ...
        uint8 bitsOrder2[8] = { 7, 2, 4, 5, 0, 6, 1, 3 };
        data.WriteBitInOrder(thisGuid, bitsOrder2);
        ...
        uint8 bytesOrder[8] = { 4, 6, 5, 3, 0, 1, 7, 2 };
...
void Unit::SendChangeCurrentVictimOpcode(HostileReference* pHostileReference)
{
    if (!getThreatManager().isThreatListEmpty())
    {
        ...
        WorldPacket data(SMSG_HIGHEST_THREAT_UPDATE, 1 + 1 + 3 + (count*13));
        ObjectGuid thisGuid = GetGUID();
        ...
        for (std::list<HostileReference*>::const_iterator itr = tlist.begin(); itr != tlist.end(); ++itr)
        {
            ...
            uint8 bitsOrder[8] = { 7, 6, 3, 2, 0, 5, 1, 4 };
...
{
    WorldPacket data(SMSG_BREAK_TARGET, 1 + 8);
    ObjectGuid unitGuid = GetGUID();

    uint8 bitsOrder[8] = { 4, 2, 5, 3, 7, 1, 6, 0 };
    data.WriteBitInOrder(unitGuid, bitsOrder);

    uint8 bytesOrder[8] = { 3, 0, 5, 7, 4, 6, 2, 1 };
...
bool Unit::IsVisionObscured(Unit* victim, SpellInfo const* spellInfo)
{
    ...
    if (victimAura != nullptr && myAura == nullptr)
    {
        if (IsFriendlyTo(victimCaster) || 
            (spellInfo->GetMaxRange(false) <= 5.0f && spellInfo->Id != 98020))
            return false;
        else
            return true;
    }
...
{
    // players cannot have holy resistance but creatures can
    if (GetTypeId() == TYPEID_PLAYER)
        if (mask == SPELL_SCHOOL_MASK_HOLY)
            return 0;
...
{
    bool isCannotTurn = HasUnitState(UNIT_STATE_CANNOT_TURN);
    if (isCannotTurn)
        return;

    if (IsCreature())
    {
        // Creature should not change its orientation while casting aoe, cone or self-casting spells
        bool keepOrientation = ToCreature()->IsShouldKeepOrientationWhileCasting();
        if (keepOrientation)
            return;
    }
...
void Unit::SetEclipsePower(int32 power, bool send)
{
    ...
    if (send)
        SendPowerUpdate(POWER_ECLIPSE, _eclipsePower);
}
...
uint32 Unit::GetHealingDoneInPastSecs(uint32 secs)
{
    ...
    for (HealDamageLog& itr : m_healDamage)
    {
        if (itr.m_type != HEAL_DONE_LOG)
            continue;

        if (GetMSTimeDiffToNow(itr.m_time) <= (secs * IN_MILLISECONDS))
            heal += itr.m_value;
    }
...
void Unit::AddHealDamageLog(uint32 value, HealDamageLogType type)
{
    for (HealDamageLog& log : m_healDamage)
    {
        // Search for log with same type, that was added in last one second
        if (log.m_type != type)
            continue;

        if (GetMSTimeDiffToNow(log.m_time) <= (1 * IN_MILLISECONDS))
        {
            log.m_value += value;
            return;
        }
    ...
    m_healDamage.emplace_back(value, getMSTime(), type);
}
    ...
    for (; *sequence != MSEEnd; ++sequence)
    {
        ...
        switch (element)
        {
            ...
            case MSEGeneric2bits0:
                data.ReadBits(2);
                break;
            case MSERemoveMovementForcesCount:
                bitcounterLoop = data.ReadBits(22);
                break;
            case MSEMovementForcesId:
...
void Unit::WriteMovementInfo(WorldPacket &data, ExtraMovementStatusElement* extras) const
{
    ...
    bool hasTransportData = mi->transport.guid != 0LL;

    ObjectGuid guid = mi->guid;
    ObjectGuid tguid = mi->transport.guid;
    ...
    for (; *sequence != MSEEnd; ++sequence)
    {
        ...
        switch (element)
        {
            ...
            case MSETransportOrientation:
                if (hasTransportData)
                    data << mi->transport.pos.GetOrientation();
                break;
            case MSETransportSeat:
                if (hasTransportData)
                    data << mi->transport.seat;
...
void Unit::RemoveSoulSwapDOT(Unit* p_Target)
{
    ...
    for (AuraEffectList::const_iterator iter = mPeriodic.begin(); iter != mPeriodic.end(); ++iter)
    {
        ...
        {
            if (AuraEffect* l_AuraEffect = l_Aura->GetEffect(l_Index))
            {
...
void Unit::ApplySoulSwapDOT(Unit* p_Target)
{
    for (SoulSwapAuraInfo const& l_Aura : m_SoulSwapDOTList)
    {
        ...
        if (Aura* l_NewAura = AddAura(l_Aura.m_ID, p_Target))
        {
            ...
            for (uint8 l_Index = 0; l_Index < MAX_EFFECTS; ++l_Index)
            {
                if (AuraEffect* l_AuraEffect = l_NewAura->GetEffect(l_Index))
                {
...
{
    if (Unit* simulacrumTarget = sObjectAccessor->GetUnit(*this, simulacrumTargetGUID))
    {
...
BattlePet::Battle* Unit::GetPetBattle() 
{ 
    return sPetBattleMgr->GetBattle(m_petBattleId);
}

void Unit::SetPetBattleId(uint64 id)
{
    m_petBattleId = id;
}

void Unit::RegisterAuraApplyTime(Aura* aura)
{
    if (!aura)
        return;

    auto itr = m_aurasCastTime.find(aura->GetId());
    if (itr == m_aurasCastTime.end())
    {
        m_aurasCastTime[aura->GetId()].push_back(aura->GetApplyTime());
        return;
    }

    itr->second.push_back(aura->GetApplyTime());
}

void Unit::UnRegisterAuraApplyTime(Aura* aura)
{
    if (!aura)
        return;
    ...
    auto itr2 = std::remove_if(list.begin(), list.end(), [&aura](time_t time) { return aura->GetApplyTime() == time; });
    if (itr2 != list.end())
        list.erase(itr2);
    ...
    m_aurasCastTime.erase(itr);
}

bool Unit::IsLatestAppliedAuras(Aura* aura) const
{
    if (!aura)
        return true;
    ...
    auto time = std::max_element(list.begin(), list.end());
    return *time == aura->GetApplyTime();
}
