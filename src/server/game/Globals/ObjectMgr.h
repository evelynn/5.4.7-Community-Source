...
struct ScriptInfo
{
    ...
    union
    {
        ...
        struct                      // SCRIPT_COMMAND_DESPAWN_CREATURE (35)
        {
            uint32 CreatureEntry;   // datalong
            uint32 DespawnDelay;    // datalong2
        } DespawnCreature;
...
enum SpellAreaTriggerSelection
{
    SELECTION_ALL       = 0,
    SELECTION_FRIENDLY  = 1,
    SELECTION_HOSTILE   = 2,
};
...
{
    SpellAreaTrigger() : 
        spell_id(0), visualRadius(3.5f), radius(3.5f), selection(SELECTION_ALL), script_id(0) {}
    SpellAreaTrigger(uint32 _spell_id, SpellAreaTriggerSelection selection, float _visualRadius, float _radius, uint32 _script_id) :
        spell_id(_spell_id), selection(SELECTION_ALL), visualRadius(_visualRadius), radius(_radius), script_id(_script_id) {}
    
    uint32 spell_id;
    float visualRadius;
    float radius;
    SpellAreaTriggerSelection selection;
    uint32 script_id;
};
...
class ObjectMgr
{
    ...
    public:
        ...
        typedef UNORDERED_MAP<uint32, SpellAreaTrigger> SpellAreaTriggerContainer;
        ...
        typedef UNORDERED_MAP<uint32, RaidEncounterData> RaidEncounterDataMap;
        ...
        uint32 GetSpellAreaTriggerScriptId(uint32 trigger_id);
        SpellAreaTrigger const* GetSpellAreaTrigger(uint32 spellId) const;
        ...
        DungeonEncounterList const* GetDungeonEncounterList(uint32 mapId, Difficulty difficulty)
        {
            ...
            // There are bad difficulties in DBC store (0)
            // So we check again
            UNORDERED_MAP<uint32, DungeonEncounterList>::const_iterator itr2 = _dungeonEncounterStore.find(MAKE_PAIR32(mapId, 0));
            if (itr2 != _dungeonEncounterStore.end())
                return &itr2->second;

            // some instances has only one diffitulty (black temple has 25ppl (4))
            // but there is 10ppl difficulty (3) in dbc
            if (difficulty == MAN25_DIFFICULTY)
            {
                UNORDERED_MAP<uint32, DungeonEncounterList>::const_iterator itr2 = _dungeonEncounterStore.find(MAKE_PAIR32(mapId, MAN10_DIFFICULTY));
                if (itr2 != _dungeonEncounterStore.end())
                    return &itr2->second;
            }
        ...
        */
        ...
        void LoadRaidEncounterData();
        RaidEncounterData const* GetRaidEncounterData(uint32 entry) const;
        RaidEncounterData* GetRaidEncounterData(uint32 entry);
    ...
    private:
        ...
        SpellAreaTriggerContainer _spellAreaTriggerStore;
    ...
    private:
        ...
        RaidEncounterDataMap m_RaidEncounterData;
