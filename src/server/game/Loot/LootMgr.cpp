...
uint32 LootStore::LoadLootTable()
{
    ...
    do
    {
        ...
        uint32  maxcount           = fields[6].GetUInt16();
...
void LootView::WriteData(ObjectGuid guid, LootType lootType, WorldPacket* data)
{
    ...
    data->WriteBit(guid[3]);
    data->WriteBit(lootGuid[3]);
    data->WriteBit((loot.gold + loot.additionalLinkedGold) == 0/*!loot.gold*/);

    size_t countPos2 = data->wpos();
    uint8 bitPos2 = data->bitpos();
    data->WriteBits(0, 20);
    ...
    switch (permission)
    {
        case GROUP_PERMISSION:
        {
            ...
            {
                if (!loot.items[i].currency && !loot.items[i].is_looted && !loot.items[i].freeforall && loot.items[i].conditions.empty() && loot.items[i].AllowedForPlayer(viewer))
                {
        ...
        case OWNER_PERMISSION:
        {
            ...
            if (permission == OWNER_PERMISSION && loot.linkedLoot.size() > 0)
            {
                //std::set<uint64> usedGuids;
                for (auto linked = loot.linkedLoot.begin(); linked != loot.linkedLoot.end(); ++linked)
                {
                    uint64 cguid = (*linked).second.creatureGUID;
                    uint32 linkedlootslot = (*linked).second.slot;

                    // TODO: need to check items but not creatures
                    // LinkedLoot structure has too few fields to identify showed items
                    //if (usedGuids.find(cguid) != usedGuids.end())
                    //    continue;
                    //usedGuids.insert(cguid);
                    if (Creature* creature = viewer->GetCreature(*viewer, cguid))
                    {
                        if (creature->loot.items.size() >= linkedlootslot)
                        {
                            LootItem* linkeditem = &creature->loot.items[linkedlootslot];
                            (*linkeditem).WriteBitDataPart(slotType, data);
                            (*linkeditem).WriteBasicDataPart((*linked).first, &itemBuff);
                            ++itemsShown;
                        }
                    }
                }
    ...
    if (hasPermission)
    {
        ...
        QuestItemMap const& lootPlayerNonQuestNonFFAConditionalItems = loot.GetPlayerNonQuestNonFFANonCurrencyConditionalItems();
    ...
    data->PutBits(itemsShown, 19, countPos, bitPos);
    data->PutBits(currenciesShown, 20, countPos2, bitPos2);
    ...
    if (loot.gold + loot.additionalLinkedGold)
        *data << uint32(loot.gold + loot.additionalLinkedGold);
...
void LootTemplate::FillAutoAssignationLoot(std::list<const ItemTemplate*>& p_ItemList) const
{
    ...
    {
        if (l_Ia->type == LOOT_ITEM_TYPE_ITEM && !l_Ia->needs_quest)
        {
...
void LootTemplate::FillAutoAssignationLootCurrency(std::list<std::pair<uint32/*currency*/, uint32/*count*/> >& currencies) const
{
    for (LootStoreItemList::const_iterator l_Ia = Entries.begin(); l_Ia != Entries.end(); ++l_Ia)
    {
        if (l_Ia->type == LOOT_ITEM_TYPE_CURRENCY)
        {
            if (l_Ia->mincountOrRef > 0)
            {
                auto currencyType = sCurrencyTypesStore.LookupEntry(l_Ia->itemid);
                if (!currencyType)
                    continue;

                currencies.push_back(std::make_pair((uint32)l_Ia->itemid, (uint32)l_Ia->mincountOrRef));
            }
        }
    }
}
...
void BonusLootProcessor::ProcessLootFor(Player* player)
{
    ProcessItemLootFor(player);
    ProcessCurrencyLootFor(player);
}

void BonusLootProcessor::ProcessItemLootFor(Player* player)
{
    if (m_LootTemplate == nullptr)
        return;

    std::list<ItemTemplate const*> l_LootTable;
    std::vector<uint32> l_Items;
    m_LootTemplate->FillAutoAssignationLoot(l_LootTable);
    if (l_LootTable.empty())
        return;

    float l_DropChance = m_GuaranteedLoot ? 100.0f : sWorld->getFloatConfig(CONFIG_LFR_DROP_CHANCE) + player->GetBonusRollFails();
    uint32 l_SpecID = player->GetLootSpecId() ? player->GetLootSpecId() : player->GetSpecializationId(player->GetActiveSpec());
    ...
    player->RemoveAurasByType(SPELL_AURA_TRIGGER_BONUS_LOOT);

    if (l_Items.empty())
    {
        int64 l_GoldAmount = urand(50 * GOLD, 100 * GOLD);
        player->IncreaseBonusRollFails();
        player->ModifyMoney(l_GoldAmount);
        player->SendDisplayToast(0, l_GoldAmount, TOAST_TYPE_MONEY, true, false);
    ...
    else
    {
        ...
        {
            player->AddItem(l_Items[0], 1);
            player->SendDisplayToast(l_Items[0], 1, TOAST_TYPE_NEW_ITEM, false, false);
            player->ResetBonusRollFails();
        }
        else
        {
            int64 l_GoldAmount = urand(50 * GOLD, 100 * GOLD);
...
void BonusLootProcessor::ProcessCurrencyLootFor(Player* player)
{
    if (m_LootTemplate == nullptr)
        return;

    std::list<std::pair<uint32/*currency*/, uint32/*count*/> > currencies;

    m_LootTemplate->FillAutoAssignationLootCurrency(currencies);

    if (!currencies.empty())
    {
        auto a = std::make_pair(1, 2);
        
        uint32 currencyId = currencies.front().first;
        uint32 currencyCount = currencies.front().second;

        player->ModifyCurrency(currencyId, currencyCount);

        player->SendDisplayToast(currencyId, currencyCount, TOAST_TYPE_NEW_CURRENCY, false, false);
    }
}

CreatureBonusLootProcessor::CreatureBonusLootProcessor(Creature* creature)
{
    m_Creature = creature;
    m_BonusLootData = sSpellMgr->GetSpellBonusLoot(m_Creature->GetEntry());
    m_LootTemplate = SelectLootTemplate();
}

void CreatureBonusLootProcessor::SendBonusLootToPlayers()
{
    if (!m_Creature || !m_BonusLootData)
        return;

    const float bonusLootRange = 100.0f;

    std::list<Player*> players;

    GetPlayersWithBonus(players, bonusLootRange);
    ...
    for (std::list<Player*>::const_iterator itr = players.begin(); itr != players.end(); ++itr)
    {
        ...
        // Prevent from multiply loot requests
        // It happens when there are several bosses (BossAI) at the encounter
        if (player->HasAura(m_BonusLootData->spellId))
            continue;

        m_Creature->CastSpell(player, m_BonusLootData->spellId, true);
    }
}

void CreatureBonusLootProcessor::ProcessLootFor(Player* player)
{
    if (!m_Creature || !m_BonusLootData)
        return;

    BonusLootProcessor::ProcessLootFor(player);

    player->ModifyCurrency(m_BonusLootData->requiredCurrency, -1, false);
    
    //player->ModifyCurrency(m_spellInfo->CurrencyID, -int32(m_spellInfo->CurrencyCount), false);
}

LootTemplate const* CreatureBonusLootProcessor::SelectLootTemplate()
{
    if (!m_BonusLootData)
        return NULL;

    switch (m_BonusLootData->lootSourceType)
    {
        case BonusLootSources::BONUS_LOOT_SOURCE_CREATURE:
            return LootTemplates_Creature.GetLootFor(m_BonusLootData->lootSourceEntry);
        case BonusLootSources::BONUS_LOOT_SOURCE_GAMEOBJECT:
            return LootTemplates_Gameobject.GetLootFor(m_BonusLootData->lootSourceEntry);
    }

    return NULL;
}

void CreatureBonusLootProcessor::GetPlayersWithBonus(std::list<Player*>& players, float range)
{
    if (!m_Creature || !m_BonusLootData)
        return;
...
bool CreatureBonusLootProcessor::PlayerWithCurrencyCheck::operator()(Player* u)
{
    if (!u->isAlive())
        return false;

    if (!u->HasCurrency(_currencyId, 1))
        return false;
...
SpellBonusLootProcessor::SpellBonusLootProcessor(uint32 spellId)
{
    m_SpellId = spellId;
    m_LootTemplate = LootTemplates_Spell.GetLootFor(m_SpellId);
}

void SpellBonusLootProcessor::ProcessLootFor(Player* player)
{
    BonusLootProcessor::ProcessLootFor(player);
}
